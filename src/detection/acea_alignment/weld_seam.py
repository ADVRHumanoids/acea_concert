...
def seam_frame_from_axis_and_surface(
    ...
    if not (np.all(np.isfinite(axis)) and np.all(np.isfinite(surface))):
        return None
    z = _unit(axis)
    if float(np.linalg.norm(z)) < _EPS:
        return None

    # Inward radial (surface -> axis), matching the node's own convention:
    #   radial_in = surface - (surface . z) z   (component of the surface point
    #   position perpendicular to the axis; points away from the camera, toward
    #   the axis line behind the visible near surface).
    radial_in = surface - float(np.dot(surface, z)) * z
    pn = float(np.linalg.norm(radial_in))
    ...
    if degenerate:
        ...
        x = -radial_in / pn  # outward surface normal (toward the camera)
