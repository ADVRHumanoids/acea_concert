#!/usr/bin/env python3
"""ACEA V16 color-OFF runtime candidate, isolated from every earlier baseline.
...
V12 preserves the v11 seam-centre correction and makes the tracker FSM
authoritative for downstream geometry. In addition to the v10 HOLD rules:

* a rejected TRACK measurement is never returned as the visual pipe mask;
* HOLD renders the last trusted cylinder without refitting it to the new frame;
* a junction lock in HOLD may use only a normal-validated frozen-cylinder strip,
  never the rejected current component; and
* ACQUIRE/LOST geometry is not exposed as a valid pipe or junction.
* HOLD uses a sensor-stamped base-link prediction, never a stale camera-frame
  lock, when the base-link lock is enabled;
* a missing prediction suppresses metric output instead of publishing a stale
  pose; and
* incompatible measurements cannot drag the persistent base-link cylinder.

V14 retains the V13 geometry experiment and adds a confirmed multi-frame
station lifecycle, LOST/reacquisition invalidation, and a short metric coast
that never bypasses pipe-end, field-of-view, or current-cylinder support gates.
V16 keeps the V15 measurement and safety contracts unchanged while removing
redundant computation from the already pipe-aligned seam search and the clean
lock-seeded cylinder refit. The node publishes observations only. It does not
command the robot.
"""
...
try:
    ...
    Duration = Time = tf2_ros = MultiThreadedExecutor = None  # type: ignore
...
try:
    from acea_seam_detector_v15 import detect_seam as _variant_a_detect_seam
except Exception:  # pragma: no cover - Variant A is optional
    _variant_a_detect_seam = None

try:
    from acea_fullframe_continuity_v15 import (
        FullFrameContinuityV15 as _FullFrameContinuityV15,
        FullFrameRecovery as _FullFrameRecovery,
    )
except Exception:  # pragma: no cover - V15 recovery is optional and opt-in
    _FullFrameContinuityV15 = None
    _FullFrameRecovery = Any  # type: ignore
...
try:
    from acea_alignment.pipe_pose_v16 import fit_pipe_pose as _fit_pipe_pose
...
def _pipe_model_innovation(
    previous: dict[str, Any],
    measured: dict[str, Any],
) -> dict[str, Any]:
    """Return sign-invariant cylinder innovation in a common frame.

    Axis points are not unique along an infinite cylinder, so the point metric
    deliberately removes displacement parallel to the previous axis. Only the
    lateral line displacement can move the physical cylinder.
    """
    result: dict[str, Any] = {
        "valid": False,
        "axis_deg": float("inf"),
    ...
    try:
        prev_axis = _normalize(np.asarray(previous["axis"], dtype=np.float64).reshape(3))
        prev_point = np.asarray(previous["axis_point"], dtype=np.float64).reshape(3)
        axis = _normalize(np.asarray(measured["axis"], dtype=np.float64).reshape(3))
        point = np.asarray(measured["axis_point"], dtype=np.float64).reshape(3)
        prev_radius = float(previous["radius_m"])
        radius = float(measured["radius_m"])
    except Exception:
        return result
    if not all(
        np.all(np.isfinite(value))
        for value in (prev_axis, prev_point, axis, point)
    ) or not all(np.isfinite(value) for value in (prev_radius, radius)):
        return result
    if float(np.linalg.norm(prev_axis)) < 0.5 or float(np.linalg.norm(axis)) < 0.5:
        return result
    if float(np.dot(axis, prev_axis)) < 0.0:
        axis = -axis
    axis_cos = float(np.clip(np.dot(axis, prev_axis), -1.0, 1.0))
    delta = point - prev_point
    lateral = delta - float(delta @ prev_axis) * prev_axis
    result.update(
        {
            "valid": True,
            "axis_deg": math.degrees(math.acos(axis_cos)),
            "lateral_m": float(np.linalg.norm(lateral)),
            "radius_m": abs(radius - prev_radius),
            "axis_aligned": axis,
        }
    )
    return result
...
def _quat_xyzw_to_R(q: Any) -> np.ndarray:
...
class SeamResult:
    ...
    # Measured width of the dark junction feature and the correction applied to
    # move the candidate from a feature edge to the feature centre.
    junction_feature_width_px: int = 0
    junction_feature_shift_px: int = 0
    junction_feature_centre_applied: bool = False
    junction_feature_centre_reason: str = ""
    junction_feature_anchor_distance_px: int = 0
    # V15 diagnostics: the frontend/temporal source that selected the final
    # measurement and whether a mature KLT track used the FOV-border policy.
    candidate_source: str = "unknown"
    klt_border_candidate_used: bool = False
    klt_border_candidate_reason: str = "not_used"
    # Current measured-depth support on the strict outer-cylinder surface.
    # Unlike strip_mask, this mask is not an analytically filled silhouette.
    measured_pipe_support_left_cols: int = 0
    measured_pipe_support_right_cols: int = 0
    measured_pipe_support_left_coverage: float = 0.0
    measured_pipe_support_right_coverage: float = 0.0
...
class OnlinePipeJunctionDetector:
    ...
    def __init__(self, params: dict[str, Any]):
        ...
        # Unlike junction_lock_x in the frozen baselines, this coordinate can
        # move outside the image.  Saturating it at x=0 made a junction that
        # had left the FOV appear to remain on the border and allowed an
        # unrelated line to reacquire it.  It is enabled only by the V15
        # preset; the legacy coordinate remains unchanged otherwise.
        self._junction_flow_x_unbounded: float | None = None
        ...
        self._debug_visual_pipe_mask: np.ndarray | None = None
        self._debug_cylinder_inlier_mask: np.ndarray | None = None
        self._debug_rejected_outliers_mask: np.ndarray | None = None
        self._current_cylinder_support_mask: np.ndarray | None = None
        self.external_pipe_lock_camera_model: dict[str, Any] | None = None
        self.external_pipe_lock_active = False
        self.external_pipe_lock_projection_available = False
        self.external_pipe_lock_source = "none"
        self.external_pipe_lock_projection_source = "not_requested"
        # Wrapper-provided pairing diagnostics. Current-frame rescue paths may
        # use these to reject an RGB/depth pair that was close only in arrival
        # time while its sensor headers describe different robot poses.
        self.external_sync_time_source = "unknown"
        self.external_rgb_depth_dt_s = float("inf")
        # V13: junction axial-station memory on the persistent base-frame
        # cylinder, projected into the camera by the wrapper at the RGB stamp.
        # The oblique close view displaces the collar's visible dark band by
        # ~5 mm (measured against projected GT), so during pipe HOLD the image
        # is not a trustworthy axial measurement; the station pinned during
        # clean TRACK is.
        self.external_junction_station_camera_xyz: np.ndarray | None = None
        self.external_junction_station_age_s: float = float("inf")
        self.external_junction_station_active = False
        self._station_strip_x: float | None = None
        self._station_override_used = False
        self._station_override_reason = "not_used"
        self._station_coast_frames = 0
        self._station_coast_used = False
        self._station_coast_reason = "not_used"
        self._station_innovation_veto_used = False
        self._station_innovation_px: float | None = None
        self._station_innovation_reason = "not_used"
        self._single_frame_proof_used = False
        self._single_frame_proof_reason = "not_used"
        self._partial_pipe_support_used = False
        self._partial_pipe_support_reason = "not_used"
        self._tracked_axial_override_used = False
        self._tracked_axial_override_reason = "not_used"
        self._measured_bilateral_override_used = False
        self._measured_bilateral_override_reason = "not_used"
        self._hard_veto_output_blocked = False
        # V15 border lifecycle. The signed KLT coordinate is allowed to leave
        # the strip; while it is outside, local argmax candidates cannot pull
        # the lock onto an unrelated reflection. Re-entry requires an
        # independent current-frame step plus inward optical flow.
        self._junction_out_of_view_side: str | None = None
        self._junction_last_border_proof_frame: int | None = None
        self._border_prearm_used = False
        self._border_prearm_reason = "not_used"
        self._border_exit_coast_used = False
        self._border_exit_coast_reason = "not_used"
        self._border_pipe_end_override_used = False
        self._border_pipe_end_override_reason = "not_used"
        self._out_of_view_reentry_used = False
        self._out_of_view_reentry_reason = "not_used"
        self._pipe_hold_depth_support_checked = False
        self._pipe_hold_depth_support_passed = False
        self._pipe_hold_depth_support_med_mm: float | None = None
        self._pipe_hold_depth_support_frac: float | None = None
        self._localize_base_model_used = False
        ...
        # V10: the axial bounds belong to the trusted cylinder state as much as
        # axis/point/radius do.  Recomputing them from a rejected HOLD frame lets
        # cloth or a support reshape the otherwise frozen cylinder silhouette.
        self._pipe_last_trusted_axial_extent: tuple[float, float] | None = None
        self._pipe_hold_mask_applied = False
        self._pipe_hold_mask_reason = "not_hold"
        self._pipe_hold_cylinder_supported = False
        self._pipe_hold_model_source = "not_hold"
        self._pipe_hold_base_prediction_required = False
        self._pipe_hold_base_prediction_available = False
        ...
        self._fullframe_continuity = None
        if (
            bool(self.params.get("junction_fullframe_continuity_enabled", False))
            and _FullFrameContinuityV15 is not None
            and cv2 is not None
        ):
            self._fullframe_continuity = _FullFrameContinuityV15(
                self._fullframe_continuity_params()
            )
        self._fullframe_recovery_used = False
        self._fullframe_recovery_reason = "not_used"
        self._fullframe_recovery_diagnostics: dict[str, Any] = {}

    def process(self, rgb: np.ndarray, depth: np.ndarray, k: np.ndarray) -> tuple[dict[str, Any], np.ndarray, np.ndarray]:
        self.processed_frame_count += 1
        # Frame-local diagnostics must never leak from the preceding accepted
        # frame into an ACQUIRE/LOST or failed-localization row.
        self._station_override_used = False
        self._station_override_reason = "not_used"
        self._station_coast_used = False
        self._station_coast_reason = "not_used"
        self._station_innovation_veto_used = False
        self._station_innovation_px = None
        self._station_innovation_reason = "not_used"
        self._single_frame_proof_used = False
        self._single_frame_proof_reason = "not_used"
        self._partial_pipe_support_used = False
        ...
        self._fullframe_recovery_used = False
        self._fullframe_recovery_reason = "not_used"
        self._fullframe_recovery_diagnostics = {}
        ...
        if (
            bool(self.params.get("enable_pipe_base_link_lock", False))
            and bool(self.params.get("pipe_base_lock_use_as_prediction", True))
            and self.external_pipe_lock_camera_model is not None
            and self.pipe_lock_model is not None
        ):
            self.pipe_lock_model = self.external_pipe_lock_camera_model
            self.pipe_lock_source = f"base_link_prediction:{self.external_pipe_lock_source}"
        try:
            ...
            recovered = self._try_fullframe_recovery(
                rgb, depth, k, tracker=None, seam=None, localization=None
            )
        ...
        try:
            ...
            recovered = self._try_fullframe_recovery(
                rgb, depth, k, tracker=tracker, seam=None, localization=None
            )
            if recovered is not None:
                return recovered
        ...
        if recovered is not None:
            return recovered
        self._canonicalize_weld_surface(status)
        return status, rgb_overlay, depth_overlay

    def _fullframe_continuity_params(self) -> dict[str, Any]:
        p = self.params
        return {
            "pipe_radius_m": float(p["pipe_radius_m"]),
            "search_radius_px": int(p["junction_fullframe_search_radius_px"]),
            "fresh_border_search_radius_px": int(
                p["junction_fullframe_fresh_border_search_radius_px"]
            ),
            "fresh_border_max_distance_px": int(
                p["junction_fullframe_fresh_border_max_distance_px"]
            ),
            "max_missed_frames": int(p["junction_fullframe_max_missed_frames"]),
            "normal_line_max_innovation_px": float(
                p["junction_fullframe_normal_line_max_innovation_px"]
            ),
            "normal_line_max_angle_delta_deg": float(
                p["junction_fullframe_normal_line_max_angle_delta_deg"]
            ),
            "normal_outlier_search_radius_px": int(
                p["junction_fullframe_normal_outlier_search_radius_px"]
            ),
            "normal_outlier_min_recovery_streak": int(
                p["junction_fullframe_normal_outlier_min_recovery_streak"]
            ),
            "normal_outlier_self_check_radius_px": int(
                p["junction_fullframe_normal_outlier_self_check_radius_px"]
            ),
            "normal_self_check_min_coverage": float(
                p["junction_fullframe_normal_self_check_min_coverage"]
            ),
            "normal_incomplete_search_radius_px": int(
                p["junction_fullframe_normal_incomplete_search_radius_px"]
            ),
            "global_candidate_search_radius_px": int(
                p["junction_fullframe_global_candidate_search_radius_px"]
            ),
            "min_strong_run_px": int(p["junction_fullframe_min_strong_run_px"]),
            "min_depth_same_count": int(
                p["junction_fullframe_min_depth_same_count"]
            ),
            "dark_threshold": float(p["junction_fullframe_dark_threshold"]),
            "strong_threshold": float(p["junction_fullframe_strong_threshold"]),
            "local_depth_layer_tol_m": float(
                p["junction_fullframe_local_depth_layer_tol_m"]
            ),
            "local_ransac_iterations": int(
                p["junction_fullframe_local_ransac_iterations"]
            ),
            "local_vote_line_tol_m": float(
                p["junction_fullframe_local_vote_line_tol_m"]
            ),
            "local_min_votes": int(p["junction_fullframe_local_min_votes"]),
            "local_min_radial_support": int(
                p["junction_fullframe_local_min_radial_support"]
            ),
            "local_radial_support_tol_m": float(
                p["junction_fullframe_local_radial_support_tol_m"]
            ),
            "local_line_support_tol_m": float(
                p["junction_fullframe_line_support_tol_m"]
            ),
            "local_line_min_side_points": int(
                p["junction_fullframe_line_min_side_points"]
            ),
            "local_line_min_side_fraction": float(
                p["junction_fullframe_line_min_side_fraction"]
            ),
            "local_line_max_side_median_m": float(
                p["junction_fullframe_line_max_side_median_m"]
            ),
            "local_max_image_axis_delta_deg": float(
                p["junction_fullframe_local_max_image_axis_delta_deg"]
            ),
            "local_independent_model_check_enabled": bool(
                p["junction_fullframe_independent_model_check_enabled"]
            ),
            "local_independent_max_center_delta_m": float(
                p["junction_fullframe_independent_max_center_delta_m"]
            ),
            "local_independent_max_axis_delta_deg": float(
                p["junction_fullframe_independent_max_axis_delta_deg"]
            ),
            "local_independent_max_side_median_m": float(
                p["junction_fullframe_independent_max_side_median_m"]
            ),
            "local_foreground_layer_guard_enabled": bool(
                p["junction_fullframe_foreground_layer_guard_enabled"]
            ),
            "local_max_foreground_range_excess_m": float(
                p["junction_fullframe_max_foreground_range_excess_m"]
            ),
            "trusted_current_model_enabled": bool(
                p["junction_fullframe_trusted_current_model_enabled"]
            ),
            "trusted_current_model_min_line_px": float(
                p["junction_fullframe_trusted_current_model_min_line_px"]
            ),
            "trusted_current_model_max_radius_delta_m": float(
                p["junction_fullframe_trusted_current_model_max_radius_delta_m"]
            ),
            "trusted_current_model_max_image_axis_delta_deg": float(
                p[
                    "junction_fullframe_trusted_current_model_max_image_axis_delta_deg"
                ]
            ),
        }

    @staticmethod
    def _fullframe_boundary_lines(
        tracker: TrackerResult | None,
    ) -> list[np.ndarray]:
        if tracker is None:
            return []
        try:
            endpoints = np.asarray(tracker.image_line_segment_uv, dtype=np.float64)
            pipe_direction = _normalize(
                np.asarray(tracker.image_direction_uv, dtype=np.float64)
            )
            tangent = np.array([-pipe_direction[1], pipe_direction[0]], dtype=np.float64)
            bbox = tracker.bbox_uv
            cross_span = max(24.0, float(bbox[3] - bbox[1] + 1) * 0.65)
            return [
                np.stack([point - cross_span * tangent, point + cross_span * tangent])
    ...
    @staticmethod
    def _candidate_line_for_strip_x(
        seam: SeamResult, candidate_x_strip_px: int
    ) -> np.ndarray:
        """Map an arbitrary current strip column back to the original image."""
        x0 = float(seam.candidate_x_rotated_px) - float(
            seam.candidate_x_strip_px
        )
        x_rotated = x0 + float(candidate_x_strip_px)
        y0, y1 = float(seam.crop_xyxy[1]), float(seam.crop_xyxy[3])
        points_rotated = np.array(
            [[x_rotated, y0], [x_rotated, y1]], dtype=np.float64
        )
        return _inverse_rotate_uv(
            points_rotated,
            (seam.rotated_mask.shape[1], seam.rotated_mask.shape[0]),
            seam.rotation_deg,
        )

    def _fullframe_global_current_candidates(
        self,
        seam: SeamResult | None,
        normal_status: dict[str, Any] | None,
    ) -> list[tuple[str, np.ndarray]]:
        """Return strictly gated current global lines for outlier recovery.

        This path is a search expansion, never an acceptance shortcut.  It is
        enabled only by the V15 simulation preset and the returned line still
        has to pass the independent full-frame RGB/depth/cylinder proof.
        """
        if not bool(
            self.params.get(
                "junction_fullframe_global_candidate_rescue_enabled", False
            )
        ):
            return []
        if seam is None or normal_status is None:
            return []
        if not bool(normal_status.get("detector_accepted", False)) or not bool(
            normal_status.get("detector_pose_complete", False)
        ):
            return []
        if self._pipe_state != "TRACK" or not self._coloroff_cyl_ok:
            return []
        if self.pipe_component_selected_model is None:
            return []
        if bool(seam.pipe_end_rejected) or bool(
            normal_status.get("junction_hard_veto_output_blocked", False)
        ):
            return []
        if str(seam.candidate_source) not in {
            "lock_local_dark",
            "lock_local_step",
            "klt_prediction",
        }:
            return []
        if str(self.external_sync_time_source) != "header":
            return []
        if abs(float(self.external_rgb_depth_dt_s)) > float(
            self.params.get("junction_fullframe_global_candidate_max_sync_dt_s", 0.04)
        ):
            return []

        candidate_x = int(seam.classical_candidate_x_strip_px)
        width = int(seam.strip_size_wh[0])
        margin = int(seam.edge_margin_px)
        if not (margin <= candidate_x < width - margin):
            return []
        if abs(candidate_x - int(seam.candidate_x_strip_px)) < int(
            self.params.get(
                "junction_fullframe_global_candidate_min_separation_px", 8
            )
        ):
            return []
        if float(seam.classical_candidate_contrast) < float(
            self.params.get(
                "junction_fullframe_global_candidate_min_contrast", 0.05
            )
        ):
            return []
        if float(seam.classical_candidate_z_score) < float(
            self.params.get("junction_fullframe_global_candidate_min_z", 8.0)
        ):
            return []
        if self._pipe_end_rejected(seam.strip_mask, candidate_x):
            return []
        line = self._candidate_line_for_strip_x(seam, candidate_x)
        if float(np.linalg.norm(line[1] - line[0])) < float(
            self.params.get(
                "junction_fullframe_global_candidate_min_line_px", 70.0
            )
        ):
            return []
        return [("global_classical_current", line)]

    def _try_fullframe_recovery(
        self,
        rgb: np.ndarray,
        depth: np.ndarray,
        k: np.ndarray,
        *,
        ...
        normal_status: dict[str, Any] | None = None,
    ) -> tuple[dict[str, Any], np.ndarray, np.ndarray] | None:
        helper = self._fullframe_continuity
        if helper is None:
            return None
        if seam is not None and bool(seam.pipe_end_rejected):
            # A vetoed candidate is not allowed to mutate continuity state;
            # otherwise a later frame could inherit a line that was never
            # safety-valid even though this frame itself emitted no pose.
            self._fullframe_recovery_reason = "blocked_by_pipe_end_veto"
            return None
        if normal_status is not None and bool(
            normal_status.get("junction_hard_veto_output_blocked", False)
        ):
            self._fullframe_recovery_reason = "blocked_by_hard_veto"
            return None
        # ``eligible`` is the normal frontend's current 2D/safety decision;
        # ``detector_accepted`` additionally requires a complete 3D pose.  An
        # eligible first CANDIDATE frame can therefore have no localization
        # yet.  The full-frame helper must still be allowed to complete that
        # frame from a fresh RGB-D + cylinder proof, while downstream output
        # remains blocked unless the resulting centre and surface are finite.
        normal_eligible = bool(
            normal_status is not None
            and normal_status.get("eligible", False)
        )
        normal_accepted = bool(
            normal_status is not None
            and normal_status.get("detector_accepted", False)
        )
        normal_complete = bool(
            normal_accepted and self._localization_complete(localization)
        )
        normal_line = None
        if seam is not None:
            try:
                normal_line = self._candidate_line_original_uv(seam)
            except Exception:
                normal_line = None
        fresh_border_allowed = bool(
            not helper.active
            and tracker is not None
            and self._pipe_state == "TRACK"
            and bool(
                self.params.get(
        ...
        boundary_lines = (
            self._fullframe_boundary_lines(tracker) if fresh_border_allowed else []
        )
        current_pipe_model = None
        if (
            bool(
                self.params.get(
                    "junction_fullframe_trusted_current_model_enabled", False
                )
            )
            and tracker is not None
            and self._pipe_state == "TRACK"
            and not self._pipe_tracker_hold_frame
            and self._coloroff_cyl_ok
            and self._coloroff_pipe_visible
            and self.pipe_component_selected_model is not None
        ):
            # This is the cylinder selected and validated from the current
            # full depth frame. The helper may use it as an independent model,
            # but never receives a HOLD/lock-only prediction.
            current_pipe_model = dict(self.pipe_component_selected_model)
            current_pipe_model["fullframe_current_model_source"] = (
                "current_track_component"
            )
        result = helper.evaluate(
            rgb,
            depth,
            k,
            normal_line_uv=normal_line,
            # The helper's legacy argument name means "normal candidate may
            # seed a current proof", not "controller-facing pose exists".
            normal_accepted=normal_eligible,
            normal_complete=normal_complete,
            boundary_lines=boundary_lines,
            current_pipe_model=current_pipe_model,
            alternative_current_lines=self._fullframe_global_current_candidates(
                seam, normal_status
            ),
        )
        self._fullframe_recovery_reason = str(result.reason)
        self._fullframe_recovery_diagnostics = dict(result.diagnostics)
        if normal_status is not None:
            normal_status["junction_fullframe_recovery_used"] = False
            normal_status["junction_fullframe_recovery_reason"] = str(result.reason)
            normal_status["junction_fullframe_recovery_diagnostics"] = dict(
                result.diagnostics
            )
        if bool(result.diagnostics.get("normal_outlier_output_blocked", False)):
            # Station arbitration (V16): the outlier veto compares the
            # normal candidate against the helper's own reference line. When
            # that reference line FAILED its current-frame cylinder proof
            # ("independent_line_not_on_cylinder"), the veto's witness is
            # itself unproven; if at the same time the metric base-frame
            # station projected at this RGB stamp is compatible with the
            # candidate (existing <=12 px strict gate, TRACK only, fresh
            # station), two independent current signals outvote the unproven
            # reference. The published pose stays the NORMAL pipeline's fully
            # proven current measurement; the station never becomes output.
            # Measured trigger: idle live frame t=31.054 (evidence z=38.5,
            # station innovation 6.4 px) was silenced by a reference that
            # failed its own proof.
            if (
                bool(
                    self.params.get(
                        "junction_fullframe_outlier_station_arbitration_enabled",
                        False,
                    )
                )
                and normal_complete
                and str(result.reason) == "independent_line_not_on_cylinder"
                and bool(self.external_junction_station_active)
                and self._station_innovation_reason == "compatible"
            ):
                if normal_status is not None:
                    normal_status["junction_fullframe_outlier_station_arbitrated"] = True
                    normal_status["junction_fullframe_recovery_reason"] = (
                        "outlier_station_arbitrated:" + str(result.reason)
                    )
                return None
            # The normal frontend produced a complete-looking pose, but its
            # line disagrees with a multi-frame sequence of independently
            # proven current RGB-D cylinders. If the current replacement cue
            # is too weak, silence this frame and release the contaminated
            # junction lock. Keeping pipe geometry is useful; publishing a
            # junction centre is not.
            self._release_junction_lock("fullframe_normal_outlier_unresolved")
            blocked = dict(normal_status or {})
            blocked.update(
                {
                    "state": "SEARCHING",
                    "detected": False,
                    ...
                    "detector_pose_complete": False,
                    "eligible": False,
                    "confidence": 0.0,
                    "reason": "fullframe_normal_outlier_unresolved",
                    "coarse_seam_center_camera_xyz_m": None,
                    "coarse_seam_visible_surface_camera_xyz_m": None,
                    "junction_lock_source": "released:fullframe_normal_outlier",
                    "junction_fullframe_recovery_used": False,
                    "junction_fullframe_recovery_reason": str(result.reason),
                    "junction_fullframe_recovery_diagnostics": dict(
                        result.diagnostics
                    ),
                    "junction_fullframe_normal_outlier_blocked": True,
                }
            )
            return blocked, rgb.copy(), _depth_visual(depth)
        normal_outlier_recovered = bool(
            result.accepted
            and result.diagnostics.get("source") == "normal_outlier_recovery"
        )
        if normal_complete and not normal_outlier_recovered:
            return None
        if not bool(result.accepted):
            return None
        return self._publish_fullframe_recovery(result, rgb, depth)

    def _publish_fullframe_recovery(
        self,
        result: Any,
        rgb: np.ndarray,
        depth: np.ndarray,
    ) -> tuple[dict[str, Any], np.ndarray, np.ndarray]:
        axis, axis_sign_flipped = self._canonical_camera_pipe_axis(
            np.asarray(result.axis_camera_xyz, dtype=np.float64).reshape(3)
        )
        axis_point = np.asarray(result.axis_point_camera_xyz_m, dtype=np.float64).reshape(3)
        center_measured = np.asarray(
            result.center_camera_xyz_m, dtype=np.float64
        ).reshape(3)
        # Use the same innovation-gated 3D filter as the regular localization
        # path. The full-frame proof is fully current, but its independently
        # fitted cylinder is otherwise published raw for one frame and creates
        # 13-34 mm spikes before the normal path returns. Filtering here does
        # not authorize a stale pose: no current proof means this method is
        # never called.
        center = self._filter_junction_center(center_measured.copy())
        surface = np.asarray(result.surface_camera_xyz_m, dtype=np.float64).reshape(3)
        radius = float(result.radius_m)
        diagnostics = dict(result.diagnostics)
        diagnostics["published_axis_sign_flipped"] = bool(axis_sign_flipped)
        diagnostics["published_axis_sign_convention"] = "camera_x_nonnegative"
        diagnostics["published_center_smoothing_shift_mm"] = float(
            np.linalg.norm(center - center_measured) * 1000.0
        )
        diagnostics["published_center_measurement_camera_xyz_m"] = [
            float(value) for value in center_measured
        ]
        residual_m = 0.001 * float(
            diagnostics.get("local_cylinder_residual_med_mm", 0.0)
        )
        model = {
            "axis": axis.copy(),
            "axis_point": axis_point.copy(),
            "radius_m": radius,
            "stand_off_m": float(center[2]),
            "residual_m": residual_m,
            "inlier_fraction": float(
                diagnostics.get("local_cylinder_support", 0)
            )
            / max(1.0, float(diagnostics.get("local_cylinder_points", 0))),
        }
        # The local proof is a full current-frame cylinder validation, so it is
        # a legitimate TRACK transition rather than output during LOST/HOLD.
        self.pipe_component_selected_model = dict(model)
        self.pipe_lock_model = dict(model)
        self.pipe_lock_source = "fullframe_current_cylinder"
        self.pipe_lock_missed_frames = 0
        self._coloroff_cyl_ok = True
        self._coloroff_cyl_reason = "fullframe_current_cylinder"
        self._coloroff_fail_streak = 0
        self._coloroff_hold_count = 0
        self._pipe_state = "TRACK"
        self._pipe_tracker_hold_frame = False
        self._coloroff_pipe_visible = True
        self._fullframe_recovery_used = True
        self._fullframe_recovery_reason = str(result.reason)

        line = np.asarray(result.line_uv, dtype=np.float64).reshape(2, 2)
        midpoint = line.mean(axis=0)
        strong_run = int(diagnostics.get("strong_run", 0))
        confidence = float(np.clip(strong_run / 20.0, 0.5, 1.0))
        status = self._no_pipe_status("fullframe_current_proof")
        status.update(
            {
                ...
                "detector_pose_complete": True,
                "eligible": True,
                "confidence": confidence,
                "reason": "fullframe_current_proof;eligible;stop_and_localize",
                "pipe_tracker_state": "TRACK",
                "coloroff_pipe_visible": True,
                "coloroff_cyl_ok": True,
                "coloroff_cyl_reason": "fullframe_current_cylinder",
                "pipe_lock_active": True,
                "pipe_lock_source": "fullframe_current_cylinder",
                "pipe_pose_fit_method": "fullframe_local_cylinder",
                "pipe_pose_radius_m": radius,
                "pipe_pose_residual_m": residual_m,
                "pipe_pose_inlier_count": int(
                    diagnostics.get("local_cylinder_support", 0)
                ),
                "pipe_pose_inlier_fraction": model["inlier_fraction"],
                "pipe_axis_camera_xyz": [float(value) for value in axis],
                "pipe_axis_point_camera_xyz_m": [
                    float(value) for value in axis_point
                ],
                "coarse_seam_center_camera_xyz_m": [
                    float(value) for value in center
                ],
                "coarse_seam_visible_surface_camera_xyz_m": [
                    float(value) for value in surface
                ],
                "coarse_seam_support_pixel_count": int(
                    diagnostics.get("local_cylinder_support", 0)
                ),
                "junction_center_method": "fullframe_local_cylinder_axis_ray",
                "candidate_line_image_uv": line.tolist(),
                "candidate_x_image_px": float(midpoint[0]),
                "candidate_source": str(diagnostics.get("source", "fullframe")),
                "candidate_streak": max(1, int(self.candidate_streak)),
                "junction_lock_source": "fullframe_current_proof",
                "junction_hard_veto_output_blocked": False,
                "pipe_end_rejected": False,
                "junction_fullframe_recovery_used": True,
                "junction_fullframe_recovery_reason": str(result.reason),
                "junction_fullframe_recovery_diagnostics": diagnostics,
            }
        )
        self._canonicalize_weld_surface(status)
        rgb_overlay = rgb.copy()
        if cv2 is not None:
            endpoints = np.rint(line).astype(np.int32)
            cv2.line(
                rgb_overlay,
                tuple(endpoints[0]),
                tuple(endpoints[1]),
            ...
            cv2.circle(
                rgb_overlay,
                tuple(np.rint(midpoint).astype(np.int32)),
                6,
                (255, 255, 0),
                -1,
                cv2.LINE_AA,
            )
        depth_overlay = _depth_visual(depth)
        return status, rgb_overlay, depth_overlay
    ...
    @staticmethod
    def _canonical_camera_pipe_axis(axis: np.ndarray) -> tuple[np.ndarray, bool]:
        """Match the normal tracker convention for full-frame cylinder fits.

        A cylinder axis is sign-ambiguous. The normal path has always selected
        the sign with non-negative camera-frame x, while V15's local full-frame
        fitter returned the raw RANSAC sign. Alternating between those paths
        therefore produced a 180-degree controller pose flip despite identical
        geometry. Keep one deterministic convention at the source.
        """
        canonical = _normalize(np.asarray(axis, dtype=np.float64).reshape(3))
        flipped = bool(canonical[0] < 0.0)
        if flipped:
            canonical = -canonical
        return canonical, flipped

    def _canonicalize_weld_surface(self, status: dict[str, Any]) -> None:
        """Use one geometric definition for the controller-facing seam point.

        The normal path historically published a measured depth sample, while
        the full-frame path published the camera-facing point of its fitted
        cylinder. Switching paths therefore moved the pose origin by up to
        68.5 mm even when the current junction centre and pipe axis agreed.

        When enabled, preserve the raw depth point for diagnostics and replace
        it with the point one fitted radius from the *current* junction centre,
        along the cylinder radial toward the camera origin. This is the same
        radial convention used by the seam-frame orientation. The method is
        strictly subtractive with respect to validity: it never creates a
        missing surface point and refuses rejected/vetoed detections.
        """
        status["weld_surface_canonicalized"] = False
        status["weld_surface_canonicalization_reason"] = "disabled"
        status["weld_surface_canonicalization_shift_mm"] = None
        if not bool(self.params.get("weld_surface_canonicalization_enabled", False)):
            return
        if not bool(status.get("detector_accepted", False)):
            status["weld_surface_canonicalization_reason"] = "detector_not_accepted"
            return
        if bool(status.get("pipe_end_rejected", False)) or bool(
            status.get("junction_hard_veto_output_blocked", False)
        ):
            status["weld_surface_canonicalization_reason"] = "safety_veto"
            return
        try:
            raw_surface = np.asarray(
                status["coarse_seam_visible_surface_camera_xyz_m"],
                dtype=np.float64,
            ).reshape(3)
            center = np.asarray(
                status["coarse_seam_center_camera_xyz_m"], dtype=np.float64
            ).reshape(3)
            axis = _normalize(
                np.asarray(status["pipe_axis_camera_xyz"], dtype=np.float64).reshape(3)
            )
            radius = float(status["pipe_pose_radius_m"])
        except (KeyError, TypeError, ValueError):
            status["weld_surface_canonicalization_reason"] = "incomplete_geometry"
            return
        if not (
            np.isfinite(raw_surface).all()
            and np.isfinite(center).all()
            and np.isfinite(axis).all()
            and np.isfinite(radius)
            and float(np.linalg.norm(axis)) > 0.99
            and radius > 1e-4
        ):
            status["weld_surface_canonicalization_reason"] = "invalid_geometry"
            return
        nominal = max(1e-4, float(self.params.get("pipe_radius_m", radius)))
        radius_margin = max(0.02, 0.30 * nominal)
        if abs(radius - nominal) > radius_margin:
            status["weld_surface_canonicalization_reason"] = "radius_out_of_bounds"
            return

        # Closest point from the camera origin to the axis through `center`.
        # Its negated perpendicular vector points from that axis toward camera.
        radial = -center + float(np.dot(center, axis)) * axis
        radial_norm = float(np.linalg.norm(radial))
        if radial_norm < 1e-8:
            status["weld_surface_canonicalization_reason"] = "radial_degenerate"
            return
        canonical = center + radius * radial / radial_norm
        if not np.isfinite(canonical).all() or canonical[2] <= 0.0:
            status["weld_surface_canonicalization_reason"] = "canonical_point_invalid"
            return

        status["coarse_seam_visible_surface_measurement_camera_xyz_m"] = [
            float(value) for value in raw_surface
        ]
        status["coarse_seam_visible_surface_camera_xyz_m"] = [
            float(value) for value in canonical
        ]
        status["weld_surface_canonicalized"] = True
        status["weld_surface_canonicalization_reason"] = "current_axis_center_radius"
        status["weld_surface_canonicalization_shift_mm"] = float(
            np.linalg.norm(canonical - raw_surface) * 1000.0
        )
    ...
    def _no_pipe_status(self, reason: str) -> dict[str, Any]:
        return {
            ...
            "junction_station_override_used": False,
            "junction_station_coast_used": False,
    ...
    def _mark_pipe_lock_missed(self, reason: str) -> None:
        ...
        if (
            ...
            and not (
                bool(self.params.get("pipe_fsm_owns_missed_release", False))
                and self._coloroff_guard_frame_active
            )
        ):
    ...
    def _update_pipe_lock(self, tracker: TrackerResult) -> None:
        if not bool(self.params["enable_pipe_temporal_lock"]):
            return
        if self._coloroff_guard_frame_active and self._pipe_state != "TRACK":
            if (
                self._pipe_state == "HOLD"
                and self.processed_frame_count != self._pipe_lock_update_count_frame
            ):
                self._pipe_lock_update_count_frame = int(self.processed_frame_count)
                self._pipe_lock_update_held += 1
            self._mark_pipe_lock_missed(f"fsm_{self._pipe_state.lower()}_blocks_model_update")
            return
        if (
    ...
    def _cylinder_azimuth_features(
        ...
        # Regressing the raw [-pi, pi] angles makes a real cylinder look
        # planar (or even produce a negative slope) whenever its visible arc
        # crosses the atan2 branch cut.  Centre phi on its circular mean and
        # express psi as phi plus their wrapped phase error.  A cylinder then
        # remains psi ~= phi with slope ~= 1 across the branch cut, while a
        # plane with near-constant normals still has slope ~= 0.
        phi_center = math.atan2(float(np.sin(phi).mean()), float(np.cos(phi).mean()))
        phi_unwrapped = np.arctan2(
            np.sin(phi - phi_center), np.cos(phi - phi_center)
        )
        normal_phase_error = np.arctan2(np.sin(psi - phi), np.cos(psi - phi))
        psi_unwrapped = phi_unwrapped + normal_phase_error
        var_phi = float(np.var(phi_unwrapped))
        slope = 0.0
        if var_phi > (math.radians(4.0) ** 2):
            slope = float(
                np.mean(
                    (phi_unwrapped - phi_unwrapped.mean())
                    * (psi_unwrapped - psi_unwrapped.mean())
                )
                / var_phi
            )
        align_frac = float(
            (np.cos(normal_phase_error) >= math.cos(math.radians(25.0))).mean()
        )
        ...
        if int(inl.sum()) >= 30:
            cov_inlier_deg = float(
                np.degrees(
                    np.percentile(phi_unwrapped[inl], 97.0)
                    - np.percentile(phi_unwrapped[inl], 3.0)
                )
            )
        return {
            "slope": slope,
            ...
            "cov_deg": float(
                np.degrees(
                    np.percentile(phi_unwrapped, 97.0)
                    - np.percentile(phi_unwrapped, 3.0)
                )
            ),
    ...
    def _release_pipe_lock_to_lost(self, reason: str) -> None:
        """Shared LOST transition: identical actions for the legacy in-branch
        release and the V13 post-render unsupported-streak release."""
        self.pipe_lock_model = None
        self.pipe_image_lock_model = None
        self.pipe_lock_source = f"released:{reason}"
        self.pipe_image_lock_source = f"released:{reason}"
        self._pipe_model_filt = None
        self._release_junction_lock(reason)
        self._coloroff_hold_count = 0
        self._coloroff_fail_streak = 0
        self._pipe_state = "LOST"

    def _junction_lock_is_mature(self) -> bool:
        if not self.junction_lock_active or self.junction_lock_start_frame is None:
            return False
        age = int(self.processed_frame_count) - int(self.junction_lock_start_frame)
        return age >= int(self.params.get("junction_mature_lock_min_age_frames", 8))

    def _mature_klt_visual_override_allowed(
        self, klt_prediction: dict[str, Any]
    ) -> bool:
        """Allow KLT to overrule a strong visual cue only at the FOV margin.

        A mature optical-flow track is useful where the ordinary frontend is
        deliberately blind because its edge margin excludes the junction. In
        the image interior, however, KLT follows generic pipe texture and can
        drift hundreds of pixels while a strong visual seam remains correct.
        V15 initially allowed that drift globally (old bag f589-596,
        f674-743); restricting the override to ``border_tracking`` preserves
        border continuity without weakening the interior visual measurement.
        """
        return bool(
            bool(self.params.get("junction_klt_prefer_mature_lock_enabled", False))
            and self._junction_lock_is_mature()
            and str(klt_prediction.get("reason")) == "tracked"
            and int(klt_prediction.get("points", 0) or 0)
            >= int(self.params["klt_min_valid_points"])
            and bool(klt_prediction.get("border_tracking", False))
        )

    def _coloroff_lock_seeded_support_ok(self) -> tuple[bool, str]:
        """Positive current-frame proof for a partial cylinder observation.

        The azimuthal normal-rotation gate is intentionally strict during
        global acquisition, but becomes under-determined when only a narrow
        cylinder arc remains in view.  In TRACK/HOLD we already have a trusted
        cylinder and the current frame has been refit from that lock.  Accept
        that partial observation only when its radius, axis and lateral line
        position remain tightly compatible with the lock and the current
        points have low residual and radial normals.  This is not a stale HOLD:
        every condition below is measured on the current depth frame.
        """
        self._partial_pipe_support_used = False
        self._partial_pipe_support_reason = "not_used"
        if not bool(self.params.get("pipe_partial_lock_support_enabled", False)):
            self._partial_pipe_support_reason = "disabled"
            return False, self._partial_pipe_support_reason
        if self.pipe_lock_model is None or self.pipe_component_selected_model is None:
            self._partial_pipe_support_reason = "model_unavailable"
            return False, self._partial_pipe_support_reason
        if not bool(self.pipe_mask_reproject_info.get("applied", False)):
            self._partial_pipe_support_reason = (
                f"reproject:{self.pipe_mask_reproject_info.get('reason', 'not_applied')}"
            )
            return False, self._partial_pipe_support_reason
        reproject_source = str(self.pipe_mask_reproject_info.get("source"))
        source_allowed = bool(
            reproject_source == "lock"
            or (
                reproject_source == "selected"
                and self.params.get(
                    "pipe_partial_lock_allow_current_selected", False
                )
            )
        )
        if not source_allowed:
            self._partial_pipe_support_reason = (
                f"source:{reproject_source}"
            )
            return False, self._partial_pipe_support_reason
        model = self.pipe_component_selected_model
        lock = self.pipe_lock_model
        if not self._pipe_model_valid_for_lock(model):
            self._partial_pipe_support_reason = "current_model_invalid"
            return False, self._partial_pipe_support_reason
        normal_fraction = self.pipe_mask_normal_fraction
        if normal_fraction is None or normal_fraction < float(
            self.params.get("pipe_partial_lock_min_normal_fraction", 0.70)
        ):
            self._partial_pipe_support_reason = (
                "normal_unavailable" if normal_fraction is None
                else f"normal={normal_fraction:.3f}"
            )
            return False, self._partial_pipe_support_reason
        try:
            ...
            lock_axis = _normalize(np.asarray(lock["axis"], dtype=np.float64).reshape(3))
            lock_point = np.asarray(lock["axis_point"], dtype=np.float64).reshape(3)
            if float(np.dot(axis, lock_axis)) < 0.0:
                axis = -axis
            axis_delta = self._axis_delta_deg(axis, lock_axis)
            delta = point - lock_point
            line_delta = float(
                np.linalg.norm(delta - float(np.dot(delta, lock_axis)) * lock_axis)
            )
            radius_delta = abs(float(model["radius_m"]) - float(lock["radius_m"]))
            residual = float(model.get("residual_m", float("inf")))
            inlier = float(model.get("inlier_fraction", 0.0))
        except Exception:
            self._partial_pipe_support_reason = "bad_model"
            return False, self._partial_pipe_support_reason
        line_limit = float(
            self.params.get("pipe_partial_lock_max_line_delta_m", 0.012)
        )
        if bool(
            self.params.get("pipe_partial_lock_motion_comp_enabled", False)
        ):
            motion_extra = abs(float(getattr(self, "klt_last_dx_px", 0.0))) * float(
                self.params.get("pipe_partial_lock_lateral_per_klt_px_m", 0.0)
            )
            line_limit += min(
                motion_extra,
                float(
                    self.params.get(
                        "pipe_partial_lock_motion_max_extra_m", 0.0
                    )
                ),
            )
        checks = (
            axis_delta <= float(self.params.get("pipe_partial_lock_max_axis_delta_deg", 3.0))
            and line_delta <= line_limit
            and radius_delta <= float(self.params.get("pipe_partial_lock_max_radius_delta_m", 0.008))
            and residual <= float(self.params.get("pipe_partial_lock_max_residual_m", 0.008))
            and inlier >= float(self.params.get("pipe_partial_lock_min_inlier_fraction", 0.60))
        )
        self._partial_pipe_support_reason = (
            f"axis={axis_delta:.2f}deg,line={line_delta*1000.0:.1f}/"
            f"{line_limit*1000.0:.1f}mm,"
            f"radius={radius_delta*1000.0:.1f}mm,res={residual*1000.0:.1f}mm,"
            f"inlier={inlier:.2f},normal={normal_fraction:.2f}"
        )
        self._partial_pipe_support_used = bool(checks)
        return bool(checks), self._partial_pipe_support_reason
    ...
    def _advance_pipe_tracker_state(
        ...
        if not cyl_ok and state in ("TRACK", "HOLD"):
            partial_ok, partial_reason = self._coloroff_lock_seeded_support_ok()
            if partial_ok:
                cyl_ok = True
                reason = f"partial_lock_support:{partial_reason}"
        ...
        if state in ("ACQUIRE", "LOST"):
            ...
            if cyl_ok and model_valid:
                ...
                self._coloroff_hold_count += 1
                self._coloroff_fail_streak += 1
                release_due = self._coloroff_hold_count >= int(
                    self.params["pipe_coloroff_release_frames"]
                )
                if release_due and not bool(
                    self.params.get("pipe_hold_release_only_when_unsupported", False)
                ):
                    # Impostor escape without color: a lock that stays
                    # non-cylindrical this long is cloth/clutter, not the pipe.
                    self._release_pipe_lock_to_lost("coloroff_not_cylindrical")
                else:
                    # V13: the release decision moves after the held-mask
                    # render, where per-frame support (normals + depth on the
                    # projected base cylinder) is known. A verifiably present
                    # pipe must not expire by wall-clock; a verifiably absent
                    # one still releases after release_frames unsupported
                    # frames.
                    self._pipe_state = "HOLD"
        ...
        if (
            ...
            and self._coloroff_cyl_ok
            and self._pipe_model_valid_for_lock(self.pipe_component_selected_model)
            and self._reproject_axial_extent is not None
        ):
            self._pipe_last_trusted_axial_extent = tuple(
                float(value) for value in self._reproject_axial_extent
            )

        if self._pipe_state == "HOLD":
            pipe_mask = self._render_held_pipe_mask(valid, depth, k)
            if bool(self.params.get("pipe_hold_release_only_when_unsupported", False)):
                if self._pipe_hold_mask_applied:
                    # Supported frame: the projected cylinder is confirmed by
                    # the current normals (or depth residual), so the pipe is
                    # demonstrably still there. Do not accumulate toward LOST.
                    self._coloroff_hold_count = 0
                elif self._coloroff_hold_count >= int(
                    self.params["pipe_coloroff_release_frames"]
                ):
                    self._release_pipe_lock_to_lost("coloroff_not_cylindrical")
                    pipe_mask = np.zeros_like(valid, dtype=bool)

        self._pipe_tracker_hold_frame = self._pipe_state == "HOLD"
        # Internal ACQUIRE candidates remain available to build the stable
        # streak, but only TRACK or a successfully rendered HOLD model is a
        # public pipe observation.  This removes startup flicker without making
        # acquisition itself slower.
        self._coloroff_pipe_visible = bool(
            self.pipe_lock_model is not None
            and (
                self._pipe_state == "TRACK"
                or (self._pipe_state == "HOLD" and self._pipe_hold_mask_applied)
            )
        )
        return pipe_mask
    ...
    def _cylinder_reproject_mask(
        ...
        *,
        ...
        axial_extent_override: tuple[float, float] | None = None,
    ) -> np.ndarray:
        ...
        source = "held_lock" if model_override is not None else "selected"
        if model is None:
            model = self.pipe_component_selected_model
        ...
        normal_axis_override = None
        if (
            bool(self.params.get("pipe_mask_reproject_reuse_cached_normal_axis", False))
            and bool(self.params["pipe_mask_reproject_refit_use_normal_axis"])
            and nrm_at is not None
            and ok_at is not None
        ):
            # The color-OFF guard has already built a current, smoothed normal
            # field at this exact sample stride. Reuse the normals belonging to
            # the final cylinder seed and solve the same normal-SVD eigenproblem
            # instead of backprojecting and filtering the whole depth image a
            # second time in pipe_pose_v16.
            normal_seed = seed_sel & ok_at
            if int(normal_seed.sum()) >= int(
                self.params["cylinder_component_min_inliers"]
            ):
                unit = np.asarray(nrm_at[normal_seed], dtype=np.float64)
                scatter = unit.T @ unit
                if np.isfinite(scatter).all():
                    _eigvals, eigvecs = np.linalg.eigh(scatter)
                    normal_axis_override = _normalize(eigvecs[:, 0])
                    if normal_axis_override[0] < 0.0:
                        normal_axis_override = -normal_axis_override
                    info["normal_axis_source"] = "current_cached_field"

        nominal = max(1e-6, float(self.params["pipe_radius_m"]))
        refit_model: dict[str, Any] = model
        run_refit = not freeze_model and (
            source == "lock" or bool(self.params["pipe_mask_reproject_refit_when_selected"])
        )
        if run_refit:
        ...
        if not freeze_model and bool(self.params["pipe_mask_reproject_known_radius_recenter"]):
        ...
        if not freeze_model and bool(self.params["enable_pipe_model_smoothing"]):
        ...
        if freeze_model and axial_extent_override is not None:
            s_lo, s_hi = axial_extent_override
            info["axial_extent_source"] = "trusted_lock"
        else:
            s_lo, s_hi = np.percentile(axial_surface, [1.0, 99.0])
            info["axial_extent_source"] = "current_surface"
    ...
    def _select_held_pipe_model(self) -> tuple[dict[str, Any] | None, str]:
        """Select the only cylinder that HOLD is allowed to expose.

        A camera-frame lock is stale as soon as the wrist moves. When the V12
        base prediction policy is enabled, HOLD therefore requires the
        base-link cylinder projected at the current sensor stamp. The legacy
        camera-frame fallback remains available only when the policy is off.
        """
        require_base = bool(
            self.params.get("pipe_hold_require_base_prediction", False)
            and self.params.get("enable_pipe_base_link_lock", False)
            and self.params.get("pipe_base_lock_use_as_prediction", True)
        )
        self._pipe_hold_base_prediction_required = require_base
        self._pipe_hold_base_prediction_available = bool(
            self.external_pipe_lock_camera_model is not None
        )
        if require_base:
            if self.external_pipe_lock_camera_model is None:
                if not self.external_pipe_lock_active:
                    return None, "base_prediction_unavailable:no_base_lock"
                return (
                    None,
                    "base_prediction_unavailable:"
                    f"{self.external_pipe_lock_projection_source}",
                )
            return dict(self.external_pipe_lock_camera_model), "base_link_prediction"
        if self.pipe_lock_model is None:
            return None, "camera_lock_unavailable"
        return dict(self.pipe_lock_model), "camera_frame_lock"

    def _render_held_pipe_mask(
        self,
        valid: np.ndarray,
        depth: np.ndarray,
        k: np.ndarray,
    ) -> np.ndarray:
        """Render only the last trusted cylinder for an FSM HOLD frame.

        V8 froze ``pipe_lock_model`` but returned the rejected current mask.
        That made a failed periodic re-anchor visibly jump onto the black cloth
        and also fed the cloth strip to the junction lock.  This path is
        deliberately prediction-only: no refit, recenter, smoothing, or axial
        extent update is allowed from the rejected frame.
        """
        empty = np.zeros_like(valid, dtype=bool)
        self._pipe_hold_mask_applied = False
        self._pipe_hold_mask_reason = "no_lock"
        held_model, held_source = self._select_held_pipe_model()
        self._pipe_hold_model_source = held_source
        if held_model is None:
            self._pipe_hold_mask_reason = held_source
            self.pipe_component_selected_model = None
            self.pipe_component_selected_image_model = None
            self.pipe_component_selection_info = {
                "method": "fsm_hold_no_timestamped_prediction",
                "component_count": None,
                "selected_label": None,
            }
            return empty

        held_mask = self._cylinder_reproject_mask(
            empty,
            valid,
            depth,
            k,
            model_override=held_model,
            freeze_model=True,
            axial_extent_override=self._pipe_last_trusted_axial_extent,
        )
        applied = bool(self.pipe_mask_reproject_info.get("applied"))
        self._pipe_hold_mask_applied = applied
        self._pipe_hold_mask_reason = str(self.pipe_mask_reproject_info.get("reason", "unknown"))

        # Every downstream 3D consumer must see the held model, never the failed
        # candidate that caused this transition.
        self.pipe_component_selected_model = held_model
        self.pipe_component_selected_image_model = None
        self.pipe_component_selection_info = {
            "method": "fsm_hold_frozen_reproject" if applied else "fsm_hold_no_safe_mask",
            "component_count": None,
            "selected_label": None,
        }
        if not applied:
        ...
        self.pipe_mask_warm_fraction = self._mask_warm_fraction(
            held_mask, self._warm_mask_current
        )
        self._update_coloroff_normal_stats(held_mask, depth, k)
        feats = self._coloroff_cyl_features
        normal_fraction = self.pipe_mask_normal_fraction
        held_ok = bool(
            feats is not None
            and normal_fraction is not None
            and normal_fraction >= float(self.params["pipe_hold_min_normal_fraction"])
            and feats["slope"] >= float(self.params["pipe_hold_min_cyl_slope"])
            and feats["align_frac"] >= float(self.params["pipe_hold_min_cyl_align_frac"])
            and feats["cov_inlier_deg"] >= float(self.params["pipe_hold_min_cyl_cov_inlier_deg"])
        )
        if feats is None or normal_fraction is None:
            held_reason = "no_features"
        elif held_ok:
            held_reason = "ok"
        else:
            held_reason = (
                f"normal={normal_fraction:.2f},slope={feats['slope']:.2f},"
                f"align={feats['align_frac']:.2f},cov={feats['cov_inlier_deg']:.0f}"
            )
        self._pipe_hold_depth_support_checked = False
        self._pipe_hold_depth_support_passed = False
        self._pipe_hold_depth_support_med_mm = None
        self._pipe_hold_depth_support_frac = None
        camera_lock_depth_check = bool(
            held_source == "camera_frame_lock"
            and self.params.get("pipe_hold_depth_support_allow_camera_lock", False)
            and self._coloroff_hold_count
            <= int(self.params.get("pipe_hold_camera_lock_max_frames", 6))
        )
        if (
            not held_ok
            and (held_source == "base_link_prediction" or camera_lock_depth_check)
            and bool(self.params.get("pipe_hold_depth_support_enabled", False))
            and normal_fraction is not None
            and normal_fraction >= float(self.params["pipe_hold_min_normal_fraction"])
        ):
            # Close-view regime: when the pipe fills the frame the visible
            # azimuth span collapses and the slope/coverage discriminator is
            # structurally blind (measured cov 39-54 deg, slope -0.10..0.33 on
            # true pipe). The base-projected cylinder is TF-exact and
            # innovation-gated, so demand instead that the MEASURED depth lies
            # on that predicted surface: cloth at a different radius fails,
            # while the true pipe measured 3.5-3.9 mm median residual at every
            # failing stamp. Radial normals are still required above.
            support = self._held_mask_depth_support(held_mask, depth, k, held_model)
            self._pipe_hold_depth_support_checked = True
            if support is not None:
                med_m, frac = support
                self._pipe_hold_depth_support_med_mm = float(med_m * 1000.0)
                self._pipe_hold_depth_support_frac = float(frac)
                if (
                    med_m <= float(self.params["pipe_hold_depth_support_max_med_m"])
                    and frac >= float(self.params["pipe_hold_depth_support_min_frac"])
                ):
                    self._pipe_hold_depth_support_passed = True
                    held_ok = True
                    held_reason = (
                        f"depth_support:med={med_m*1000.0:.1f}mm,frac={frac:.2f}"
                    )
        self._pipe_hold_cylinder_supported = bool(held_ok)
        if not held_ok:
            # A ray/cylinder intersection alone is not evidence that the pipe is
            # still visible: a nearby cloth fold can occupy the projected volume.
            # Require the current normal field to retain cylindrical azimuthal
            # rotation before exposing the held mask or coasting its junction.
            self._pipe_hold_mask_applied = False
            self._pipe_hold_mask_reason = f"unsupported:{held_reason}"
        else:
            self._pipe_hold_mask_reason = held_reason
        return held_mask

    def _held_mask_depth_support(
        self,
        ...
        """Median radial residual and inlier fraction of measured depth against
        the held cylinder, over the projected mask (strided)."""
        try:
            axis = _normalize(np.asarray(model["axis"], dtype=np.float64).reshape(3))
            axis_point = np.asarray(model["axis_point"], dtype=np.float64).reshape(3)
            radius = float(model["radius_m"])
            ...
            return None
        stride = max(1, int(self.params["pipe_mask_reproject_sample_stride"]))
        sel = held_mask[::stride, ::stride] & np.isfinite(depth[::stride, ::stride]) & (
            depth[::stride, ::stride] > 0.0
        )
        ys_s, xs_s = np.nonzero(sel)
        if ys_s.size < 50:
            return None
        ys_v = ys_s * stride
        xs_v = xs_s * stride
        ...
        q = points - axis_point[None, :]
        qd = q @ axis
        radial = np.sqrt(np.maximum(np.einsum("ij,ij->i", q, q) - qd * qd, 0.0))
        resid = np.abs(radial - radius)
        tol = float(self.params.get("pipe_hold_depth_support_tol_m", 0.010))
        return float(np.median(resid)), float((resid <= tol).mean())
    ...
    def _track_pipe(self, depth: np.ndarray, k: np.ndarray) -> TrackerResult:
        ...
        foreground_mode = str(self.params["pipe_foreground_mode"]).strip().lower()
        if foreground_mode == "valid_depth_components":
            # In sparse synthetic scenes, invalid (Inf) pixels are the
            # background and nearly every finite pixel belongs to the pipe.
            # Splitting those finite depths with k-means cuts the curved pipe
            # surface into near/far arcs before the cylinder fit. Preserve all
            # finite components here and let the existing cylinder selection
            # reject geometry that is not pipe-like.
            pipe_mask_raw = valid.copy()
            finite_depths = depth[valid]
            threshold_info = {
                "method": "valid_depth_components",
                "near_center_m": (
                    float(np.median(finite_depths)) if finite_depths.size else 0.0
                ),
                "far_center_m": (
                    float(np.max(finite_depths)) if finite_depths.size else 0.0
                ),
                "threshold_m": max_depth,
                "sample_count": int(finite_depths.size),
            }
        elif foreground_mode == "kmeans_near":
            threshold_info = _depth_threshold_kmeans(
                depth, valid, int(self.params["sample_stride"])
            )
            pipe_mask_raw = valid & (depth <= float(threshold_info["threshold_m"]))
        else:
            raise ValueError(
                "pipe_foreground_mode must be 'kmeans_near' or "
                f"'valid_depth_components', got {foreground_mode!r}"
            )
        ...
        need_debug_masks = bool(self.params.get("publish_pipe_debug_masks", False))
        need_measured_support = bool(
            self.params.get("junction_measured_bilateral_support_enabled", False)
        )
        if need_debug_masks or need_measured_support:
            self._update_pipe_debug_masks(tracker, valid, depth, k)
            if not need_debug_masks:
                self._debug_visual_pipe_mask = None
                self._debug_cylinder_inlier_mask = None
                self._debug_rejected_outliers_mask = None
        else:
            self._debug_visual_pipe_mask = None
            self._debug_cylinder_inlier_mask = None
            self._debug_rejected_outliers_mask = None
            self._current_cylinder_support_mask = None
        return tracker

    def _update_pipe_debug_masks(
        self,
        tracker: TrackerResult,
        valid: np.ndarray,
        depth: np.ndarray,
        ...
        raw_visual = np.asarray(tracker.pipe_mask, dtype=bool)
        visual = (
            raw_visual
            if bool(getattr(self, "_coloroff_pipe_visible", True))
            else np.zeros_like(raw_visual, dtype=bool)
        )
        inlier = np.zeros_like(visual, dtype=bool)
        strict_support = np.zeros_like(visual, dtype=bool)
        model = self.pipe_component_selected_model
        if model is None and not self._pipe_tracker_hold_frame:
            model = self.pipe_lock_model
        try:
            axis = _normalize(np.asarray(model["axis"], dtype=np.float64).reshape(3))
            axis_point = np.asarray(model["axis_point"], dtype=np.float64).reshape(3)
            ...
            self._debug_visual_pipe_mask = visual
            self._debug_cylinder_inlier_mask = inlier
            self._debug_rejected_outliers_mask = visual & valid
            self._current_cylinder_support_mask = strict_support
            return
        ys, xs = np.nonzero(visual & valid)
        if xs.size:
            points = _backproject(depth, xs, ys, k)
            q = points - axis_point[None, :]
            q2 = np.einsum("ij,ij->i", q, q)
            qd = q @ axis
            radial = np.sqrt(np.maximum(q2 - qd * qd, 0.0))
            tol = max(
                float(self.params.get("pipe_mask_reproject_radius_tol_abs_m", 0.03)),
                float(self.params.get("pipe_mask_reproject_radius_tol_rel", 0.25)) * max(radius, 1e-6),
            )
            ok = np.abs(radial - radius) <= tol
            inlier[ys[ok], xs[ok]] = True
            strict_tol = float(
                self.params.get("junction_measured_bilateral_radius_tol_m", 0.010)
            )
            strict_ok = np.abs(radial - radius) <= strict_tol
            strict_support[ys[strict_ok], xs[strict_ok]] = True
        self._debug_visual_pipe_mask = visual
        self._debug_cylinder_inlier_mask = inlier
        self._debug_rejected_outliers_mask = visual & valid & ~inlier
        self._current_cylinder_support_mask = strict_support
    ...
    def _variant_a_params(self) -> dict[str, Any]:
        ...
        return {
            ...
            "backend": str(self.params["variant_a_backend"]),
        }

    def _feature_centre_x(
        self,
        profile: np.ndarray,
        valid: np.ndarray,
        candidate_x: int,
    ) -> dict[str, Any]:
        """Recentre the candidate on the middle of the dark junction feature.

        The seam cue is `residual = background - profile`, where `background` is
        a `background_window_px` (41) median of the column profile. A dark
        feature stays visible to that median only while it is a minority inside
        the window. A 5 mm marker (10 px measured) is; an 18 mm collar (36 px
        measured) is not, so it drags the background down with it, the residual
        collapses across its interior and survives only at its two edges. Every
        thin-line frontend then locks onto an edge rather than the seam.

        Measured 2026-07-17: all 27 accepted collar poses were bimodal at the
        collar edges (+/-9 mm = half the collar width), none within 4 mm of the
        seam, while the 5 mm modes stayed within 1-2 mm. Widening the window
        alone does not fix it, because the residual then becomes a flat plateau
        whose argmax is an arbitrary column.

        Estimating the pipe surface globally instead of locally makes the whole
        feature visible again; its centre is the seam. Returns the original
        candidate unchanged when the feature is implausible or clipped, so a
        shadow cannot capture the seam.
        """
        result: dict[str, Any] = {
            "applied": False,
            "candidate_x": int(candidate_x),
            "width_px": 0,
            "anchor_distance_px": 0,
            "reason": "disabled",
        }
        if not bool(self.params.get("junction_feature_centre_enabled", False)):
            return result

        finite = np.isfinite(profile) & valid
        if not finite.any():
            result["reason"] = "no_valid_columns"
            return result
        ...
        # Robust bright pipe surface: the pipe dominates the strip, the feature
        # is a minority, so a high percentile is the surface even when the
        # feature is wide enough to defeat the local median.
        surface = float(np.percentile(profile[finite], 70.0))
        # A step frontend can put the candidate a few columns on the bright side
        # of the boundary. Estimate the feature floor in a bounded neighbourhood
        # instead of requiring the candidate pixel itself to be dark. The 10th
        # percentile tolerates a single dark noise sample while still seeing the
        # measured collar when only 3-5 of the 13 local columns are inside it.
        anchor_search_px = max(
            0, int(self.params.get("junction_feature_anchor_search_px", 6))
        )
        local_lo = max(0, int(candidate_x) - anchor_search_px)
        local_hi = min(width, int(candidate_x) + anchor_search_px + 1)
        local_values = profile[local_lo:local_hi][finite[local_lo:local_hi]]
        if local_values.size == 0:
            result["reason"] = "no_local_support"
            return result
        floor = float(np.percentile(local_values, 10.0))
        depth = surface - floor
        min_depth = float(self.params.get("junction_feature_min_depth", 0.02))
        if depth < min_depth:
            result["reason"] = f"shallow_feature depth={depth:.3f}"
            return result

        # Half-depth crossing: independent of the feature's absolute darkness,
        # so it treats a black band and a dark-orange collar the same way.
        level = surface - 0.5 * depth
        dark = finite & (profile <= level)
        max_width = int(self.params.get("junction_feature_max_width_px", 60))
        min_width = int(self.params.get("junction_feature_min_width_px", 6))
        interior = np.where(valid)[0]
        interior_lo = int(interior[0]) if interior.size else 0
        interior_hi = int(interior[-1]) if interior.size else width - 1

        # Enumerate complete dark runs. A run is eligible only when it lies at
        # most anchor_search_px from the evidence column and is geometrically
        # plausible. This recovers candidates just outside the collar without
        # allowing an arbitrary distant shadow to capture the seam.
        nearby_runs: list[tuple[int, int, int, int]] = []
        plausible_runs: list[tuple[int, int, int, int]] = []
        index = 0
        while index < width:
            if not dark[index]:
                index += 1
                continue
            lo = index
            while index + 1 < width and dark[index + 1]:
                index += 1
            hi = index
            run_width = hi - lo + 1
            if candidate_x < lo:
                distance = lo - candidate_x
            elif candidate_x > hi:
                distance = candidate_x - hi
            else:
                distance = 0
            if distance <= anchor_search_px:
                run = (lo, hi, run_width, int(distance))
                nearby_runs.append(run)
                clipped = lo <= interior_lo or hi >= interior_hi
                if min_width <= run_width <= max_width and not clipped:
                    plausible_runs.append(run)
            index += 1

        if not plausible_runs:
            if not nearby_runs:
                result["reason"] = "no_nearby_dark_run"
                return result
            nearest = min(
                nearby_runs,
                key=lambda run: (run[3], abs((run[0] + run[1]) / 2.0 - candidate_x)),
            )
            lo, hi, run_width, distance = nearest
            result["width_px"] = int(run_width)
            result["anchor_distance_px"] = int(distance)
            if run_width < min_width:
                result["reason"] = f"too_narrow {run_width}<{min_width}"
            elif run_width > max_width:
                result["reason"] = f"too_wide {run_width}>{max_width}"
            else:
                result["reason"] = "feature_clipped"
            return result

        lo, hi, run_width, distance = min(
            plausible_runs,
            key=lambda run: (
                run[3],
                abs((run[0] + run[1]) / 2.0 - candidate_x),
                -run[2],
            ),
        )
        result["width_px"] = int(run_width)
        ...
        centre = int(round(0.5 * (lo + hi)))
        result["applied"] = True
        result["candidate_x"] = int(np.clip(centre, 0, width - 1))
        result["shift_px"] = int(result["candidate_x"] - candidate_x)
        result["reason"] = "centred" if distance == 0 else "centred_from_nearby_run"
        return result
    ...
    def _lock_local_visual_candidate(
        self,
        residual: np.ndarray,
        dark_z: np.ndarray,
        step_abs: np.ndarray,
        step_z: np.ndarray,
        valid: np.ndarray,
        safe_cols: np.ndarray,
    ) -> tuple[int, str] | None:
        """Find current seam evidence near a mature lock after KLT loss.

        A temporary occlusion can remove every optical-flow point while the
        junction reappears close to its last bounded prediction. The global
        frontend may then elect a stronger unrelated step elsewhere. Search
        only the ordinary lock radius and require a normal dark/step gate.
        This path never creates a fresh lock.
        """
        if not bool(
            self.params.get("junction_lock_local_visual_fallback_enabled", False)
        ):
            return None
        if not self._junction_lock_is_mature() or self.junction_lock_x is None:
            return None
        if self._junction_out_of_view_side is not None:
            return None
        predicted = float(self.junction_lock_x) + float(
            self.junction_lock_velocity_px
        )
        width = int(residual.size)
        radius = float(self.params["junction_lock_search_radius_px"])
        columns = np.arange(width, dtype=np.float64)
        selection = (
            valid
            & safe_cols
            & (np.abs(columns - predicted) <= radius)
            & np.isfinite(dark_z)
            & np.isfinite(step_z)
        )
        ...
        if indices.size == 0:
            return None
        evidence = np.maximum(dark_z, step_z)
        minimum = float(
            self.params.get("junction_lock_local_visual_min_evidence_z", 3.0)
        )
        for x_raw in indices[np.argsort(evidence[indices])[::-1]]:
            x = int(x_raw)
            if float(evidence[x]) < minimum:
                break
            step_ok = bool(
                float(step_z[x])
                >= float(self.params["step_edge_reacquire_min_z"])
                and float(step_abs[x]) >= float(self.params["step_edge_min_abs"])
            )
            dark_ok = bool(
                float(dark_z[x])
                >= float(self.params["junction_lock_min_reacquire_z"])
                and float(max(0.0, residual[x]))
                >= float(self.params["junction_lock_min_reacquire_contrast"])
            )
            if step_ok:
                return x, "lock_local_step"
            if dark_ok:
                return x, "lock_local_dark"
        return None
    ...
    def _detect_seam(self, rgb: np.ndarray, depth: np.ndarray, tracker: TrackerResult) -> SeamResult:
        ...
        strict_support_full = self._current_cylinder_support_mask
        if (
            strict_support_full is None
            or strict_support_full.shape != tracker.pipe_mask.shape
        ):
            strict_support_full = np.zeros_like(tracker.pipe_mask, dtype=bool)
        if cv2 is not None:
            ...
            rotated_strict_support = cv2.warpAffine(
                strict_support_full.astype(np.uint8),
                rot_m,
                (img_w, img_h),
            ...
            strict_support_image = PilImage.fromarray(
                strict_support_full.astype(np.uint8) * 255, mode="L"
            )
            ...
            rotated_strict_support = np.asarray(
                strict_support_image.rotate(
                    angle_deg,
        ...
        strip_strict_support = rotated_strict_support[y0:y1 + 1, x0:x1 + 1]
        ...
        klt_prediction = self._klt_predict_junction_x(
            gray,
            strip_mask,
            evidence_z_profile,
            valid,
            tracking_valid=step_data_ok,
        )
        klt_border_candidate_used = False
        klt_border_candidate_reason = "not_used"
        if (
            klt_prediction["available"]
            and self.junction_lock_active
            and self._pipe_state in ("TRACK", "HOLD")
        ):
            ...
            prefer_mature_klt = self._mature_klt_visual_override_allowed(
                klt_prediction
            )
            if not visual_disagrees or prefer_mature_klt:
                candidate_x = klt_candidate_x
                if bool(klt_prediction.get("border_tracking", False)):
                    candidate_source = "klt_border_prediction"
                    klt_border_candidate_used = True
                    klt_border_candidate_reason = "mature_local_search"
                else:
                    candidate_source = "klt_prediction"
                variant_a_classical_fallback_used = False
        elif self.junction_lock_active:
            local_visual = self._lock_local_visual_candidate(
                residual,
                dark_z_profile,
                step_profile,
                step_z_profile,
            ...
            if local_visual is not None:
                candidate_x, candidate_source = local_visual
                variant_a_classical_fallback_used = False
        ...
        measured_pipe_support = self._pipe_support_around_candidate(
            strip_strict_support, candidate_x
        )
        pipe_end_rejected = bool(pipe_support["pipe_end_rejected"])
        if candidate_source == "klt_border_prediction" and pipe_end_rejected:
            min_cols = int(self.params["rgb_temporal_pipe_end_min_side_columns_px"])
            min_cov = float(
                self.params.get("junction_klt_border_min_visible_side_coverage", 0.85)
            )
            missing_left = int(pipe_support["left_cols"]) < min_cols
            missing_right = int(pipe_support["right_cols"]) < min_cols
            visible_side_ok = bool(
                (
                    missing_left
                    and not missing_right
                    and float(pipe_support["right_coverage"]) >= min_cov
                )
                or (
                    missing_right
                    and not missing_left
                    and float(pipe_support["left_coverage"]) >= min_cov
                )
            )
            predicted_x = klt_prediction.get("predicted_x")
            prediction_ok = bool(
                predicted_x is not None
                and abs(float(candidate_x) - float(predicted_x))
                <= float(self.params.get("junction_klt_border_max_innovation_px", 6.0))
            )
            local_dark = bool(
                float(max(0.0, residual[candidate_x]))
                >= float(self.params.get("junction_klt_border_min_contrast", 0.015))
                and float(dark_z_profile[candidate_x])
                >= float(self.params.get("junction_klt_border_min_z", 3.0))
            )
            local_step = bool(
                float(step_profile[candidate_x])
                >= float(self.params.get("junction_klt_border_min_step_abs", 0.015))
                and float(step_z_profile[candidate_x])
                >= float(self.params.get("junction_klt_border_min_step_z", 3.0))
            )
            border_proof = bool(
                self._junction_lock_is_mature()
                and self._coloroff_cyl_ok
                and visible_side_ok
                and prediction_ok
                and (local_dark or local_step)
            )
            if border_proof:
                pipe_end_rejected = False
                klt_border_candidate_reason = "accepted_border_proof"
            else:
                klt_border_candidate_reason = (
                    "failed_border_proof:"
                    f"side={visible_side_ok},prediction={prediction_ok},"
                    f"dark={local_dark},step={local_step}"
                )
        ...
        if mode == "rgb_temporal":
            ...
            if variant_a_classical_fallback_used:
            ...
            elif candidate_source in ("step_edge", "lock_local_step"):
                ...
                va_reason = (
                    "lock_local_step_reacquire"
                    if candidate_source == "lock_local_step"
                    else "step_edge_fallback"
                )
            elif candidate_source == "lock_local_dark":
                va_seam = True
                va_sig = float(dark_z_profile[candidate_x])
                va_reason = "lock_local_dark_reacquire"
            elif candidate_source in ("klt_prediction", "klt_border_prediction"):
        ...
        # Report the feature's centre, not the edge the frontends scored.
        #
        # Every cue above is a thin-line/step extremum, and the residual that
        # defines them collapses inside a feature wider than the background
        # window, so their extremum sits on an edge. The evidence genuinely
        # lives there, so all scoring and gating above deliberately stay on
        # `candidate_x`; only the reported column is corrected. This keeps
        # acceptance identical to v10 and isolates the position bias.
        #
        # candidate_x_strip_px also feeds the jump/innovation gates, which is
        # wanted: on an 18 mm collar the extremum flips between the two edges
        # (measured 19/8 split, ~24 px apart) and those flips are themselves a
        # source of candidate_jump rejections. Tracking the stable centre
        # removes them.
        feature_centre = self._feature_centre_x(profile, valid, candidate_x)
        junction_feature_width_px = int(feature_centre.get("width_px", 0))
        junction_feature_shift_px = 0
        junction_feature_centre_applied = bool(feature_centre.get("applied", False))
        junction_feature_centre_reason = str(feature_centre.get("reason", ""))
        junction_feature_anchor_distance_px = int(
            feature_centre.get("anchor_distance_px", 0)
        )
        candidate_x_out = candidate_x
        if junction_feature_centre_applied:
            junction_feature_shift_px = int(feature_centre.get("shift_px", 0))
            candidate_x_out = int(feature_centre["candidate_x"])

        return SeamResult(
            candidate_x_strip_px=candidate_x_out,
            candidate_x_rotated_px=int(x0 + candidate_x_out),
            # Always retain the frontend's current raw measurement. Previous
            # versions filled this only after acceptance/smoothing, which made
            # a fail-closed out-of-view latch unable to inspect the first
            # genuine border candidate on re-entry.
            candidate_x_raw_strip_px=int(candidate_x_out),
            ...
            measured_pipe_support_left_cols=int(measured_pipe_support["left_cols"]),
            measured_pipe_support_right_cols=int(measured_pipe_support["right_cols"]),
            measured_pipe_support_left_coverage=float(
                measured_pipe_support["left_coverage"]
            ),
            measured_pipe_support_right_coverage=float(
                measured_pipe_support["right_coverage"]
            ),
            accepted=accepted,
            ...
            junction_feature_width_px=junction_feature_width_px,
            junction_feature_shift_px=junction_feature_shift_px,
            junction_feature_centre_applied=junction_feature_centre_applied,
            junction_feature_centre_reason=junction_feature_centre_reason,
            junction_feature_anchor_distance_px=junction_feature_anchor_distance_px,
            step_fallback_used=bool(
                candidate_source in ("step_edge", "lock_local_step")
            ),
            candidate_source=candidate_source,
            klt_border_candidate_used=klt_border_candidate_used,
            klt_border_candidate_reason=klt_border_candidate_reason,
        )
    ...
    def _klt_predict_junction_x(
        ...
        valid: np.ndarray,
        tracking_valid: np.ndarray | None = None,
    ) -> dict[str, Any]:
        ...
        h = canon_h
        if (
            bool(self.params.get("junction_unbounded_flow_enabled", False))
            and self._junction_flow_x_unbounded is not None
        ):
            prev_x = float(self._junction_flow_x_unbounded)
        else:
        ...
        if (
            bool(self.params.get("junction_unbounded_flow_enabled", False))
            and self._junction_out_of_view_side is not None
            and not (0.0 <= predicted < float(w))
        ):
            # Once the tracked junction has crossed the image boundary, a
            # clipped local search would scan unrelated in-frame texture and
            # can jump back onto a reflection (old bag f461-f462). Keep
            # measuring pipe optical flow at the nearest boundary, but expose
            # no KLT candidate until the dedicated re-entry proof succeeds.
            self._klt_update_reference(
                gray_u8,
                strip_mask,
                float(np.clip(predicted, 0.0, float(max(w - 1, 0)))),
            )
            self.klt_last_points = points_now
            return {
                "available": False,
                "reason": "junction_out_of_view",
                "candidate_x": None,
                "predicted_x": float(predicted),
                "dx_px": float(self.klt_last_dx_px),
                "points": int(points_now),
                "border_tracking": False,
            }
        ...
        border_tracking = False
        anchor = int(round(np.clip(predicted, 0, max(w - 1, 0))))
        if (
            bool(self.params.get("junction_klt_border_search_enabled", False))
            and tracking_valid is not None
            and tracking_valid.shape == valid.shape
            and 0.0 <= predicted < float(w)
            and not bool(valid[anchor])
        ):
            # The regular candidate mask deliberately excludes a large image
            # margin.  A mature lock approaching the FOV boundary needs the
            # raw data-backed columns there; acceptance still requires local
            # seam evidence and the dedicated tracked-border proof below.
            local_valid = tracking_valid.astype(bool).copy()
        ...
        if not local_valid.any():
            ...
            return {
                ...
                "dx_px": float(self.klt_last_dx_px),
                "points": int(points_now),
                "border_tracking": bool(border_tracking),
            }
    ...
    def _column_profile(self, gray: np.ndarray, mask: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        # NumPy emits RuntimeWarning for columns that are intentionally all
        # NaN before the validity mask/interpolation below handles them.
    ...
    def _station_strip_column(self, seam: SeamResult) -> float | None:
        """Project the externally provided base-frame junction station (already
        transformed to CAMERA coordinates at this frame's RGB stamp) into the
        current seam strip and return its column.

        Returns None whenever the station is inactive, too old, behind the
        camera, or lands outside the valid strip interior — the caller then
        falls back to the legacy lock+velocity prediction.
        """
        if not bool(self.params.get("junction_base_station_enabled", False)):
            return None
        if not self.external_junction_station_active:
            return None
        point = self.external_junction_station_camera_xyz
        if point is None:
            return None
        if float(self.external_junction_station_age_s) > float(
            self.params.get("junction_station_max_age_s", 30.0)
        ):
            return None
        p = np.asarray(point, dtype=np.float64).reshape(3)
        if not np.isfinite(p).all() or p[2] <= 0.02:
            return None
        ...
        if width <= 0:
            return None
        # Forward rotation original->rotated (inverse of _inverse_rotate_uv):
        # rotate by -angle about the same image centre used by PIL rotate.
        img_w = int(seam.rotated_mask.shape[1])
        img_h = int(seam.rotated_mask.shape[0])
        ccx = (img_w - 1) * 0.5
        ccy = (img_h - 1) * 0.5
        theta = math.radians(float(seam.rotation_deg))
        dx = u - ccx
        dy = v - ccy
        x_rot = math.cos(theta) * dx + math.sin(theta) * dy + ccx
        x0 = float(seam.candidate_x_rotated_px) - float(seam.candidate_x_strip_px)
        x_strip = x_rot - x0
        margin = float(seam.edge_margin_px)
        if not (margin <= x_strip < width - margin):
            return None
        return float(x_strip)

    def _station_consistency_veto(self, seam: SeamResult) -> str | None:
        """Reject a visual measurement that contradicts a confirmed station.

        The station is fixed on the static base-frame pipe and is projected at
        this RGB stamp. Once confirmed, a high image score is not permission to
        jump to another feature: the 2026-07-17 tone-band replay contained one
        evidence-z=28.4 candidate 50.4 mm away from the station. The wrapper
        correctly refused to update the station, but the detector still
        published that candidate because the old innovation veto applied only
        below an evidence threshold.

        This method is purely subtractive. It neither accepts nor publishes a
        pose; the existing station-coast path may replace the vetoed candidate
        only after its independent cylinder, FOV and pipe-end checks pass.
        """
        self._station_innovation_veto_used = False
        self._station_innovation_px = None
        self._station_innovation_reason = "not_used"
        if not bool(
            self.params.get("junction_station_strict_innovation_gate_enabled", False)
        ):
            self._station_innovation_reason = "disabled"
            return None
        if (
            not self.external_junction_station_active
            or self._station_strip_x is None
        ):
            self._station_innovation_reason = "station_unavailable"
            return None
        if self._pipe_state != "TRACK":
            self._station_innovation_reason = f"pipe_state_{self._pipe_state.lower()}"
            return None
        innovation = abs(
            float(seam.candidate_x_strip_px) - float(self._station_strip_x)
        )
        self._station_innovation_px = float(innovation)
        maximum = float(
            self.params.get("junction_station_max_innovation_px", 12.0)
        )
        if innovation <= maximum:
            self._station_innovation_reason = "compatible"
            return None
        self._station_innovation_veto_used = True
        self._station_innovation_reason = (
            f"station_innovation={innovation:.1f}px>{maximum:.1f}px"
        )
        return self._station_innovation_reason

    def _single_frame_geometric_dark_proof(self, seam: SeamResult) -> tuple[bool, str]:
        """Independent color-OFF proof used only while the pipe is acquiring.

        Stable-cylinder streaks protect global acquisition from cloth, but on
        the old real bag the junction is already in view in frame zero.  The
        first eight true frames have a separation that the unobservable cloth
        frames do not: a current valid task-radius cylinder plus a thin dark
        feature with contrast >=0.05, robust z >=4 and RGB-dark score >=0.8.
        Requiring all of those conditions is a positive proof; it does not
        lower the normal fresh-evidence threshold for any other candidate.
        """
        self._single_frame_proof_used = False
        self._single_frame_proof_reason = "not_used"
        if not bool(self.params.get("junction_single_frame_proof_enabled", False)):
            self._single_frame_proof_reason = "disabled"
            return False, self._single_frame_proof_reason
        if not self._coloroff_guard_frame_active:
            self._single_frame_proof_reason = "not_coloroff"
            return False, self._single_frame_proof_reason
        if self._pipe_state not in ("ACQUIRE", "LOST"):
            self._single_frame_proof_reason = f"pipe_state_{self._pipe_state.lower()}"
            return False, self._single_frame_proof_reason
        model_ok = bool(
            self._coloroff_cyl_ok
            and self._pipe_model_valid_for_lock(self.pipe_component_selected_model)
        )
        normal = self.pipe_mask_normal_fraction
        checks = {
            "model": model_ok,
            "normal": normal is not None and normal >= float(
                self.params.get("junction_single_frame_min_normal_fraction", 0.70)
            ),
            "frontend": bool(
                seam.accepted
                and seam.local_candidate_accepted
                and seam.visual_frontend_accepted
            ),
            "contrast": float(seam.candidate_contrast) >= float(
                self.params.get("junction_single_frame_min_contrast", 0.05)
            ),
            "z": float(seam.candidate_z_score) >= float(
                self.params.get("junction_single_frame_min_z", 4.0)
            ),
            "dark": float(seam.rgb_dark_score) >= float(
                self.params.get("junction_single_frame_min_dark_score", 0.80)
            ),
            "not_end": not seam.pipe_end_rejected,
            "border": self._candidate_not_border(seam),
        }
        passed = all(checks.values())
        failed = [name for name, ok in checks.items() if not ok]
        self._single_frame_proof_used = bool(passed)
        self._single_frame_proof_reason = (
            "accepted" if passed else "failed:" + ",".join(failed)
        )
        return bool(passed), self._single_frame_proof_reason
    ...
    def _update_state(self, tracker: TrackerResult, seam: SeamResult) -> dict[str, Any]:
        ...
        self._station_strip_x = self._station_strip_column(seam)
        ...
        single_frame_proof, single_frame_proof_reason = (
            self._single_frame_geometric_dark_proof(seam)
        )
        ...
        if eligible and self._junction_out_of_view_side is not None:
            # A global frontend can find unrelated in-frame lines while the
            # signed junction track is physically outside (old bag f463).
            # Force every candidate through the dedicated inward-motion +
            # physical-step re-entry proof before normal eligibility can
            # rewrite the lock.
            eligible = False
            mask_gate_reason = "out_of_view_reentry_required"
        if eligible:
            ...
            if strip_evidence < fresh_min_evidence and not single_frame_proof:
                eligible = False
                mask_gate_reason = f"no_strip_evidence={strip_evidence:.2f}"
        if eligible:
            station_veto = self._station_consistency_veto(seam)
            if station_veto is not None:
                eligible = False
                mask_gate_reason = station_veto
        if (
            eligible
            and self.junction_lock_active
            and self.junction_lock_x is not None
            and float(self.params["junction_lock_innovation_min_evidence_z"]) > 0.0
        ):
            predicted_x = float(self.junction_lock_x) + float(self.junction_lock_velocity_px)
            if (
                bool(self.params.get("junction_station_use_for_prediction", False))
                and self._station_strip_x is not None
            ):
                # The base-frame station projected at THIS frame's stamp is a
                # metric prediction of where the junction column must be; the
                # lock+velocity extrapolation lags during camera sweeps and
                # mislabels correct candidates as weak innovations. Thresholds
                # are unchanged: only the reference of the innovation moves.
                predicted_x = float(self._station_strip_x)
            innovation_px = abs(float(seam.candidate_x_strip_px) - predicted_x)
            min_innovation_evidence = float(
                self.params["junction_lock_innovation_min_evidence_z"]
            )
            if (
                innovation_px > float(self.params["junction_lock_innovation_gate_px"])
                and strip_evidence < min_innovation_evidence
            ):
                eligible = False
                mask_gate_reason = (
                    f"weak_lock_innovation={innovation_px:.1f}px"
                    f",evidence={strip_evidence:.2f}<{min_innovation_evidence:.2f}"
                )
        ...
        if eligible and self._coloroff_guard_frame_active:
            ...
            if self._pipe_state != "TRACK" and not single_frame_proof:
                eligible = False
                mask_gate_reason = f"pipe_tracker_{self._pipe_state.lower()}"
            elif self.pipe_lock_model is None and not single_frame_proof:
                eligible = False
        ...
        if eligible:
            ...
            if single_frame_proof:
                self.candidate_streak = max(
                    self.candidate_streak + 1,
                    int(self.params["min_confirm_frames"]),
                )
            else:
                self.candidate_streak += 1
            ...
            lock_used, lock_reason = self._try_hold_junction_lock(
                seam,
                geometry_ok,
                measurement_veto_reason=mask_gate_reason,
            )
            if lock_used:
                eligible = True
                confidence_ok = True
        ...
        return {
            ...
            "junction_lock_source": self.junction_lock_source,
            "junction_station_coast_used": bool(self._station_coast_used),
            "junction_station_coast_reason": self._station_coast_reason,
            "junction_station_coast_frames": int(self._station_coast_frames),
            "junction_single_frame_proof_used": bool(single_frame_proof),
            "junction_single_frame_proof_reason": single_frame_proof_reason,
        }
    ...
    def _update_junction_lock(self, seam: SeamResult) -> None:
        ...
        self._junction_flow_x_unbounded = float(fused)
        ...
        self._prearm_border_exit(seam)

    def _prearm_border_exit(self, seam: SeamResult) -> bool:
        """Remember a strong current junction just before the excluded margin.

        The ordinary frontend stops being authoritative inside ``edge_margin``.
        Without this pre-arm, the first one or two low-contrast border frames
        have no recent positive proof even though the immediately preceding
        in-frame measurement was valid. The proof is bounded in time and is
        consumed only by the existing KLT + current-cylinder border path.
        """
        self._border_prearm_used = False
        if not bool(self.params.get("junction_border_prearm_enabled", False)):
            self._border_prearm_reason = "disabled"
            return False
        if (
            not self._junction_lock_is_mature()
            or self._pipe_state != "TRACK"
            or not self._coloroff_cyl_ok
        ):
            self._border_prearm_reason = "lock_or_pipe_not_proven"
            return False
        if seam.pipe_end_rejected:
            self._border_prearm_reason = "image_pipe_end"
            return False
        if not seam.local_candidate_accepted or not seam.visual_frontend_accepted:
            self._border_prearm_reason = "visual_candidate_not_accepted"
            return False
        evidence = float(seam.candidate_evidence_z)
        min_evidence = float(
            self.params.get("junction_border_prearm_min_evidence_z", 3.0)
        )
        if evidence < min_evidence:
            self._border_prearm_reason = (
                f"weak_evidence={evidence:.2f}<{min_evidence:.2f}"
            )
            return False
        width = int(seam.strip_size_wh[0])
        x = float(seam.candidate_x_strip_px)
        reach = float(seam.edge_margin_px) + float(
            self.params.get("junction_border_prearm_extra_px", 24.0)
        )
        distance = min(x, float(max(width - 1, 0)) - x)
        if width <= 0 or distance > reach:
            self._border_prearm_reason = f"interior_distance={distance:.1f}>{reach:.1f}"
            return False
        self._junction_last_border_proof_frame = int(self.processed_frame_count)
        self._border_prearm_used = True
        self._border_prearm_reason = (
            f"armed:x={x:.1f},distance={distance:.1f},evidence={evidence:.2f}"
        )
        return True
    ...
    def _release_junction_lock(self, reason: str) -> tuple[bool, str]:
        ...
        self._junction_flow_x_unbounded = None
        self._junction_out_of_view_side = None
        self._junction_last_border_proof_frame = None
        self.junction_lock_velocity_px = 0.0
        self.junction_lock_streak = 0
    ...
    def _tracked_border_candidate_ok(self, seam: SeamResult) -> tuple[bool, str]:
        """Positive proof for a mature junction crossing an image boundary."""
        if seam.candidate_source != "klt_border_prediction":
            return False, "not_border_track"
        if not self._junction_lock_is_mature() or not self._coloroff_cyl_ok:
            return False, "lock_or_cylinder_not_mature"
        if str(seam.klt_status) != "tracked" or seam.klt_predicted_x_strip_px is None:
            return False, "klt_unavailable"
        innovation = abs(
            float(seam.candidate_x_strip_px)
            - float(seam.klt_predicted_x_strip_px)
        )
        prediction_ok = innovation <= float(
            self.params.get("junction_klt_border_max_innovation_px", 6.0)
        )
        dark_ok = bool(
            float(seam.candidate_contrast)
            >= float(self.params.get("junction_klt_border_min_contrast", 0.015))
            and float(seam.candidate_z_score)
            >= float(self.params.get("junction_klt_border_min_z", 3.0))
        )
        step_ok = bool(
            float(seam.candidate_step_abs)
            >= float(self.params.get("junction_klt_border_min_step_abs", 0.015))
            and float(seam.candidate_step_z)
            >= float(self.params.get("junction_klt_border_min_step_z", 3.0))
        )
        min_cols = int(self.params["rgb_temporal_pipe_end_min_side_columns_px"])
        min_cov = float(
        ...
        left_present = int(seam.pipe_support_left_cols) >= min_cols
        right_present = int(seam.pipe_support_right_cols) >= min_cols
        if left_present and right_present:
            support_ok = bool(
                float(seam.pipe_support_left_coverage) >= min_cov
                and float(seam.pipe_support_right_coverage) >= min_cov
            )
        elif left_present:
            support_ok = float(seam.pipe_support_left_coverage) >= min_cov
        elif right_present:
            support_ok = float(seam.pipe_support_right_coverage) >= min_cov
        else:
            support_ok = False
        passed = bool(prediction_ok and support_ok and (dark_ok or step_ok))
        return passed, (
            f"innovation={innovation:.1f}px,support={support_ok},"
            f"dark={dark_ok},step={step_ok}"
        )

    def _border_flow_region(self, seam: SeamResult) -> str:
        """Classify the signed KLT coordinate for border-continuity routing.

        ``border_tracking`` describes where KLT searched for the feature; it
        does not guarantee that the resulting coordinate is still at an image
        boundary.  Treating every such candidate as a border exit discarded
        valid interior measurements with ``flow_not_near_border``.
        """
        width = int(seam.strip_size_wh[0])
        flow_x = (
            None
            if self._junction_flow_x_unbounded is None
            else float(self._junction_flow_x_unbounded)
        )
        if width <= 0 or flow_x is None or not np.isfinite(flow_x):
            return "invalid"
        if not (0.0 <= flow_x < float(width)):
            return "out_of_view"
        transition_slack = float(
            self.params.get("junction_border_transition_slack_px", 2.0)
        )
        if (
            flow_x < float(seam.edge_margin_px) + transition_slack
            or flow_x
            >= float(width - seam.edge_margin_px) - transition_slack
        ):
            return "near_border"
        return "interior"

    def _publish_border_exit_continuity(self, seam: SeamResult) -> bool:
        """Publish a current KLT position while a proven junction exits FOV.

        Strong local evidence establishes the border episode. At most a small
        number of following weak frames may use current-frame optical flow,
        current cylinder validity and one-sided pipe support. Publication
        stops exactly when the signed coordinate leaves the image; it never
        clamps the junction to the border.
        """
        self._border_exit_coast_used = False
        if not bool(
            self.params.get("junction_border_exit_continuity_enabled", False)
        ):
            self._border_exit_coast_reason = "disabled"
            return False
        if (
            not self._junction_lock_is_mature()
            or self._pipe_state != "TRACK"
            or not self._coloroff_cyl_ok
            or str(seam.klt_status) != "tracked"
        ):
            self._border_exit_coast_reason = "lock_pipe_or_flow_invalid"
            return False
        if int(seam.klt_points) < int(
            self.params.get("junction_border_exit_min_klt_points", 20)
        ):
            self._border_exit_coast_reason = f"klt_points={int(seam.klt_points)}"
            return False
        ...
        flow_region = self._border_flow_region(seam)
        if flow_region in ("invalid", "out_of_view"):
            self._border_exit_coast_reason = "flow_out_of_view"
            return False
        if flow_region == "interior":
            self._border_exit_coast_reason = "flow_not_near_border"
            return False
        ...
        side_supported = bool(
            (
                int(seam.pipe_support_left_cols) >= min_cols
                and float(seam.pipe_support_left_coverage) >= min_cov
            )
            or (
                int(seam.pipe_support_right_cols) >= min_cols
                and float(seam.pipe_support_right_coverage) >= min_cov
            )
        )
        if not side_supported:
            self._border_exit_coast_reason = "visible_pipe_side_unsupported"
            return False

        strong, strong_reason = self._tracked_border_candidate_ok(seam)
        if strong:
            self._junction_last_border_proof_frame = int(self.processed_frame_count)
            # A strong border frame is a current physical measurement, not a
            # missed frame. Keeping the old accumulated count released a valid
            # lock after 12 continuously observed border frames (old bag
            # f502-510), exactly as the junction reversed direction.
            self.junction_lock_missed_frames = 0
        last = self._junction_last_border_proof_frame
        age = None if last is None else int(self.processed_frame_count) - int(last)
        if not strong and (
            age is None
            or age
            > int(self.params.get("junction_border_exit_max_weak_frames", 2))
        ):
            self._border_exit_coast_reason = (
                f"no_recent_border_proof:{strong_reason},age={age}"
            )
            return False

        if seam.pipe_end_rejected:
            # This flag is the image-space one-sided-support verdict, not the
            # metric axial pipe-end veto handled by _junction_axial_support_ok.
            # At a true image-boundary exit, the missing side is outside the
            # field of view. Resolve that verdict only after the dedicated
            # proof above has established all of the following on the current
            # frame: mature pre-existing junction lock, valid cylinder, KLT
            # flow, supported visible side, and either a strong local feature
            # or a strong feature no more than two frames old. The metric
            # pipe-end and remembered-end vetoes remain hard and are checked
            # independently before any pose can be emitted.
            seam.pipe_end_rejected = False
            self._border_pipe_end_override_used = True
            self._border_pipe_end_override_reason = (
                f"resolved_image_border:"
                f"{'strong' if strong else 'recent_strong'},age={age}"
            )

        # On a strong frame the measured local line is best; on the following
        # weak frames the only current location evidence is optical flow.
        output_x = float(seam.candidate_x_strip_px) if strong else flow_x
        output_x = float(np.clip(output_x, 0.0, float(max(width - 1, 0))))
        raw_x = int(seam.candidate_x_strip_px)
        seam.candidate_x_raw_strip_px = raw_x
        seam.candidate_x_strip_px = int(round(output_x))
        seam.candidate_x_rotated_px = int(
            seam.crop_xyxy[0] + seam.candidate_x_strip_px
        )
        ...
        self.junction_lock_x = float(output_x)
        ...
        seam.negative_gate_reason = "junction_border_exit_continuity"
        self.junction_lock_source = "border_exit_continuity"
        self._border_exit_coast_used = True
        self._border_exit_coast_reason = (
            f"accepted:{'strong' if strong else 'flow'},age={age},x={output_x:.1f}"
        )
        return True

    def _out_of_view_reentry_proof(self, seam: SeamResult) -> tuple[bool, str]:
        """Require inward motion and a new physical step before re-entry."""
        if self._junction_out_of_view_side is None:
            return False, "not_out_of_view"
        if (
            self._pipe_state != "TRACK"
            or not self._coloroff_cyl_ok
            or str(seam.klt_status) not in ("junction_out_of_view", "tracked")
        ):
            return False, "pipe_or_flow_invalid"
        raw_x = int(seam.candidate_x_raw_strip_px)
        width = int(seam.strip_size_wh[0])
        if raw_x < 0 or width <= 0:
            return False, "no_fresh_global_candidate"
        border_limit = int(seam.edge_margin_px) + int(
            self.params.get("junction_reentry_border_slack_px", 12)
        )
        if self._junction_out_of_view_side == "left":
            border_ok = raw_x <= border_limit
            inward = float(seam.klt_dx_px) >= float(
                self.params.get("junction_reentry_min_inward_dx_px", 2.0)
            )
            side_cov = float(seam.pipe_support_right_coverage)
            side_cols = int(seam.pipe_support_right_cols)
        else:
            border_ok = raw_x >= width - 1 - border_limit
            inward = float(seam.klt_dx_px) <= -float(
                self.params.get("junction_reentry_min_inward_dx_px", 2.0)
            )
            side_cov = float(seam.pipe_support_left_coverage)
            side_cols = int(seam.pipe_support_left_cols)
        step_ok = bool(
            float(seam.candidate_step_abs)
            >= float(self.params.get("junction_reentry_min_step_abs", 0.020))
            and float(seam.candidate_step_z)
            >= float(self.params.get("junction_reentry_min_step_z", 1.5))
        )
        support_ok = bool(
            side_cols >= int(self.params["rgb_temporal_pipe_end_min_side_columns_px"])
            and side_cov
            >= float(
                self.params.get("junction_klt_border_min_visible_side_coverage", 0.85)
            )
        )
        passed = bool(border_ok and inward and step_ok and support_ok)
        return passed, (
            f"side={self._junction_out_of_view_side},raw={raw_x},"
            f"dx={float(seam.klt_dx_px):.1f},step={float(seam.candidate_step_abs):.3f}/"
            f"{float(seam.candidate_step_z):.1f},support={side_cov:.2f}"
        )

    def _publish_out_of_view_reentry(self, seam: SeamResult) -> bool:
        self._out_of_view_reentry_used = False
        passed, reason = self._out_of_view_reentry_proof(seam)
        self._out_of_view_reentry_reason = reason
        if not passed:
            return False
        measured = float(seam.candidate_x_raw_strip_px)
        seam.candidate_x_strip_px = int(round(measured))
        seam.candidate_x_rotated_px = int(
            seam.crop_xyxy[0] + seam.candidate_x_strip_px
        )
        self.junction_lock_x = measured
        self._junction_flow_x_unbounded = measured
        self.junction_lock_velocity_px = float(seam.klt_dx_px)
        self.junction_lock_missed_frames = 0
        self.junction_lock_confidence = max(
            float(seam.confidence),
            float(self.params["junction_lock_min_confidence"]),
        )
        self.junction_lock_source = "out_of_view_reentry"
        self._junction_out_of_view_side = None
        self._junction_last_border_proof_frame = int(self.processed_frame_count)
        seam.accepted = True
        seam.local_candidate_accepted = True
        seam.visual_frontend_accepted = True
        seam.rgb_dark_accepted = True
        seam.negative_gate_reason = "junction_out_of_view_reentered"
        self._out_of_view_reentry_used = True
        self._out_of_view_reentry_reason = f"accepted:{reason}"
        return True

    def _try_hold_junction_lock(
        self,
        seam: SeamResult,
        geometry_ok: bool,
        measurement_veto_reason: str | None = None,
    ) -> tuple[bool, str]:
        if not bool(self.params["enable_junction_lock"]):
            return False, "disabled"

        # V10: the pipe FSM owns the validity of every downstream measurement.
        # ACQUIRE/LOST has no confirmed cylinder.  HOLD rejected the current
        # candidate, so seam evidence is allowed only after the frozen lock has
        # been re-rendered and independently confirmed by the current normal
        # field.  This preserves real-junction recall during one-frame reanchors
        # without letting the rejected cloth mask self-confirm (v8 f1242-1252).
        if self._coloroff_guard_frame_active and self._pipe_state in ("ACQUIRE", "LOST"):
            if self.junction_lock_active:
                return self._release_junction_lock(f"pipe_tracker_{self._pipe_state.lower()}")
            return False, f"pipe_tracker_{self._pipe_state.lower()}"
        if self._coloroff_guard_frame_active and self._pipe_state == "HOLD":
            if not self.junction_lock_active or self.junction_lock_x is None:
                return False, "not_locked"
            if not geometry_ok:
                return self._release_junction_lock("geometry_changed")
            if not self._pipe_hold_mask_applied:
                self.junction_lock_missed_frames += 1
                if self.junction_lock_missed_frames > int(self.params["junction_lock_max_missed_frames"]):
                    return self._release_junction_lock("missed_too_long")
                return False, f"hold_mask_invalid:{self._pipe_hold_mask_reason}"
            # Safe held mask: continue through the normal lock logic below.  Any
            # reacquire measurement now comes from the frozen cylinder strip,
            # not from the rejected current component.
        ...
        if self._junction_out_of_view_side is not None:
            # Keep only the internal trajectory hypothesis alive while the
            # junction is outside. No pose is published by this extension;
            # it exists so the dedicated re-entry proof can distinguish the
            # returning junction from an unrelated fresh acquisition.
            max_missed = max(
                max_missed,
                int(self.params.get("junction_out_of_view_max_frames", 30)),
            )
        ...
        if (
            ...
            predicted = float(self._junction_flow_x_unbounded) + float(
                self.junction_lock_velocity_px
            )
        else:
        ...
        if (
            ...
            # Metric prediction from the base-frame station at this stamp: the
            # search window follows the true junction during camera sweeps
            # instead of the stale lock+velocity extrapolation.
            predicted = float(self._station_strip_x)
        measured_i = int(seam.candidate_x_strip_px)
        safety_veto = bool(
            self.params["junction_lock_respect_safety_veto"]
            and measurement_veto_reason
            and measurement_veto_reason.startswith(
                (
                    "weak_lock_innovation=",
                    "station_innovation=",
                    "pipe_end_axial_support ",
                    "near_remembered_pipe_end",
                )
            )
        )
        if safety_veto:
            # A strict gate must not be bypassed by the relaxed lock-reacquire
            # path. The old order correctly diagnosed weak innovations and
            # pipe ends, then immediately fused the same rejected pixel as a
            # measurement, producing 20-123 mm axial pose errors. KLT coast is
            # still permitted because it does not consume the vetoed pixel.
            hard_geometric_veto = str(measurement_veto_reason).startswith(
                ("pipe_end_axial_support ", "near_remembered_pipe_end")
            )
            self._coast_lock_with_flow(seam)
            if hard_geometric_veto:
                # Never turn an explicit pipe-end verdict into a published
                # pose.  V14 did exactly that through KLT coast on seven old-
                # bag frames.  If the extent verdict is a close-view artifact,
                # _tracked_bilateral_junction_proof must resolve it *before*
                # this branch; otherwise output is fail-closed.
                self._hard_veto_output_blocked = True
                return False, f"hard_measurement_veto:{measurement_veto_reason}"
            if self._junction_out_of_view_side is not None:
                if self._publish_out_of_view_reentry(seam):
                    return True, f"out_of_view_reentry_after_veto:{measurement_veto_reason}"
                return False, f"out_of_view:{self._out_of_view_reentry_reason}"
            if self._publish_station_coast(seam):
                return True, f"station_coasted_after_veto:{measurement_veto_reason}"
            if self._publish_coasted_lock(seam):
                return True, f"coasted_after_veto:{measurement_veto_reason}"
            return False, f"measurement_veto:{measurement_veto_reason}"
        if self._junction_out_of_view_side is not None:
            self._coast_lock_with_flow(seam)
            if self._publish_out_of_view_reentry(seam):
                return True, "out_of_view_reentry"
            return False, f"out_of_view:{self._out_of_view_reentry_reason}"
        if (
            seam.candidate_source == "klt_border_prediction"
            and self._border_flow_region(seam) != "interior"
        ):
            # Border candidates must preserve the independent signed KLT
            # prediction. Sending a strong border measurement through the
            # ordinary fusion path resets that coordinate to the clipped
            # image extremum and makes it leave the FOV two frames too early.
            self._coast_lock_with_flow(seam)
            if self._publish_border_exit_continuity(seam):
                return True, "border_exit_continuity"
            return False, f"border_exit_rejected:{self._border_exit_coast_reason}"
        search_radius = float(self.params["junction_lock_search_radius_px"])
        if abs(float(measured_i) - predicted) > search_radius:
            self._coast_lock_with_flow(seam)
            if self._publish_station_coast(seam):
                return True, "station_coasted_measurement_far"
            if self._publish_coasted_lock(seam):
                return True, "coasted"
        ...
        if not visual_reacquire:
            ...
            if self._publish_border_exit_continuity(seam):
                return True, "border_exit_continuity"
            if self._publish_station_coast(seam):
                return True, "station_coasted_no_visual_reacquire"
            if self._publish_coasted_lock(seam):
                return True, "coasted"
            return False, "no_visual_reacquire"

        if not (seam.edge_margin_px <= measured_i < width - seam.edge_margin_px):
            border_ok, border_reason = self._tracked_border_candidate_ok(seam)
            if not border_ok:
                return self._release_junction_lock("out_of_view")
            seam.klt_border_candidate_reason = f"accepted:{border_reason}"
    ...
    def _publish_station_coast(self, seam: SeamResult) -> bool:
        """Use the confirmed base-frame station for a short visual dropout.

        The station is not a replacement for evidence acquisition. It can be
        consumed only after a visual lock exists, while the current frame still
        supports the same cylinder, at a projected column safely inside the
        strip and away from every remembered/current pipe end. The consecutive
        frame bound prevents an old station from becoming an unbounded phantom.
        """
        self._station_coast_used = False
        if not bool(self.params.get("junction_station_coast_enabled", False)):
            self._station_coast_reason = "disabled"
            return False
        if not self.junction_lock_active or self.junction_lock_x is None:
            self._station_coast_reason = "junction_not_locked"
            return False
        if (
            not self.external_junction_station_active
            or self.external_junction_station_camera_xyz is None
            or self._station_strip_x is None
        ):
            self._station_coast_reason = "station_unavailable"
            return False
        max_frames = max(
            0, int(self.params.get("junction_station_coast_max_frames", 3))
        )
        if self._station_coast_frames >= max_frames:
            self._station_coast_reason = "coast_limit"
            return False
        if bool(
            self.params.get("junction_station_coast_require_pipe_support", True)
        ):
            if self._pipe_state == "HOLD":
                pipe_supported = bool(
                    self._pipe_hold_mask_applied
                    and self._pipe_hold_cylinder_supported
                )
            elif self._pipe_state == "TRACK":
                pipe_supported = bool(
                    self.pipe_component_selected_model is not None
                    and (not self._coloroff_guard_frame_active or self._coloroff_cyl_ok)
                )
            else:
                pipe_supported = False
            if not pipe_supported:
                self._station_coast_reason = f"pipe_unsupported:{self._pipe_state}"
                return False

        width = int(seam.strip_size_wh[0])
        margin = int(seam.edge_margin_px) + int(
            self.params.get("junction_station_coast_edge_margin_px", 8)
        )
        x = float(self._station_strip_x)
        if width <= 0 or not (margin <= x < width - margin):
            self._station_coast_reason = "station_out_of_view"
            return False
        x_i = int(round(x))
        if self._pipe_end_rejected(seam.strip_mask, x_i):
            self._station_coast_reason = "station_pipe_end_support"
            return False

        model = self.external_pipe_lock_camera_model or self.pipe_component_selected_model
        comps = None if model is None else self._pipe_axis_model_components_from_model(model)
        if comps is not None and self._pipe_end_memory:
            point = np.asarray(
                self.external_junction_station_camera_xyz, dtype=np.float64
            ).reshape(3)
            s_station = float(np.dot(point - comps[1], comps[0]))
            if self._remembered_end_near(s_station) is not None:
                self._station_coast_reason = "station_near_remembered_pipe_end"
                return False

        previous = float(self.junction_lock_x)
        measured_velocity = x - previous
        self.junction_lock_velocity_px = (
            0.7 * self.junction_lock_velocity_px + 0.3 * measured_velocity
        )
        self.junction_lock_x = x
        self._junction_flow_x_unbounded = float(x)
        self.junction_lock_confidence = max(
            self.junction_lock_confidence
            * float(self.params["junction_lock_confidence_decay"]),
            float(self.params["junction_lock_min_confidence"]),
        )
        seam.candidate_x_raw_strip_px = int(seam.candidate_x_strip_px)
        seam.candidate_x_strip_px = x_i
        seam.candidate_x_rotated_px = int(seam.crop_xyxy[0] + x_i)
        seam.confidence = float(self.junction_lock_confidence)
        seam.negative_gate_reason = "junction_station_coasted"
        self.junction_lock_source = "station_coasted"
        self._station_coast_frames += 1
        self._station_coast_used = True
        self._station_coast_reason = "accepted"
        return True
    ...
    def _coast_lock_with_flow(self, seam: SeamResult) -> None:
        ...
        if not self.junction_lock_active or self.junction_lock_x is None:
            return
        if str(seam.klt_status) not in ("tracked", "junction_out_of_view"):
            return
        ...
        if not np.isfinite(dx) or abs(dx) > max_dx:
            return
        width = int(seam.strip_size_wh[0])
        if bool(self.params.get("junction_unbounded_flow_enabled", False)):
            current = (
                float(self._junction_flow_x_unbounded)
                if self._junction_flow_x_unbounded is not None
                else float(self.junction_lock_x)
            )
            updated = current + dx
            self._junction_flow_x_unbounded = float(updated)
            self.junction_lock_x = float(updated)
            if width > 0:
                if updated < 0.0:
                    self._junction_out_of_view_side = "left"
                elif updated >= float(width):
                    self._junction_out_of_view_side = "right"
        else:
            self.junction_lock_x = float(
                np.clip(self.junction_lock_x + dx, 0.0, float(max(width - 1, 0)))
            )
    ...
    def _tracked_bilateral_junction_proof(self, seam: SeamResult) -> tuple[bool, str]:
        """Prove that a mature tracked feature is not a newly seen pipe end.

        The metric axial extent can shorten at grazing depth/FOV cutoffs even
        though the RGB pipe mask still contains the same junction on both
        sides.  A mature track may override that *measurement artifact* only
        when current cylinder geometry is valid, KLT predicts the same column,
        and both image-space sides are densely pipe-supported.  A lock born on
        a real end is still rejected by the early end-memory check below.
        """
        if not bool(self.params.get("junction_tracked_bilateral_override_enabled", False)):
            return False, "disabled"
        if not self._junction_lock_is_mature():
            return False, "lock_not_mature"
        if self._pipe_state != "TRACK" or not self._coloroff_cyl_ok:
            return False, f"pipe_{self._pipe_state.lower()}"
        if seam.pipe_end_rejected:
            return False, "image_pipe_end"
        if str(seam.klt_status) != "tracked" or seam.klt_predicted_x_strip_px is None:
            return False, "klt_unavailable"
        innovation = abs(
            float(seam.candidate_x_strip_px)
        ...
        max_innovation = float(
            self.params.get("junction_tracked_bilateral_max_klt_innovation_px", 8.0)
        )
        min_coverage = float(
            self.params.get("junction_tracked_bilateral_min_coverage", 0.90)
        )
        passed = bool(
            innovation <= max_innovation
            and float(seam.pipe_support_left_coverage) >= min_coverage
            and float(seam.pipe_support_right_coverage) >= min_coverage
            and seam.local_candidate_accepted
            and seam.visual_frontend_accepted
        )
        return passed, (
            f"innovation={innovation:.1f}px,"
            f"coverage={float(seam.pipe_support_left_coverage):.2f}/"
            f"{float(seam.pipe_support_right_coverage):.2f}"
        )

    def _measured_bilateral_junction_proof(self, seam: SeamResult) -> tuple[bool, str]:
        """Prove current outer-cylinder surface on both sides of a visual cue.

        This is deliberately stronger than the projected visual-mask proof:
        every supporting pixel comes from the current depth image and lies
        within a tight radial residual of the current cylinder. A physical
        pipe end therefore cannot borrow support from a projected silhouette,
        an inner wall, or a stale lock. The proof is used only to resolve an
        under-measured metric extent after a mature lock; it never creates a
        lock and never overrides an image-space pipe-end rejection.
        """
        if not bool(
            self.params.get("junction_measured_bilateral_support_enabled", False)
        ):
        ...
        if str(seam.candidate_source) not in {
            ...
            return False, f"non_visual_source:{seam.candidate_source}"
        if not seam.local_candidate_accepted or not seam.visual_frontend_accepted:
            return False, "visual_candidate_not_accepted"
        min_evidence = float(
            self.params.get("junction_measured_bilateral_min_evidence_z", 1.25)
        )
        if float(seam.candidate_evidence_z) < min_evidence:
            return False, (
                f"weak_evidence={float(seam.candidate_evidence_z):.2f}"
                f"<{min_evidence:.2f}"
            )
        min_cols = int(self.params["rgb_temporal_pipe_end_min_side_columns_px"])
        left_cols = int(seam.measured_pipe_support_left_cols)
        right_cols = int(seam.measured_pipe_support_right_cols)
        if left_cols < min_cols or right_cols < min_cols:
            return False, f"short_window={left_cols}/{right_cols}<{min_cols}"
        min_coverage = float(
            self.params.get("junction_measured_bilateral_min_coverage", 0.65)
        )
        left_cov = float(seam.measured_pipe_support_left_coverage)
        right_cov = float(seam.measured_pipe_support_right_coverage)
        passed = bool(left_cov >= min_coverage and right_cov >= min_coverage)
        return passed, (
            f"measured_coverage={left_cov:.2f}/{right_cov:.2f},"
            f"cols={left_cols}/{right_cols},"
            f"evidence={float(seam.candidate_evidence_z):.2f}"
        )
    ...
    def _junction_axial_support_ok(self, seam: SeamResult) -> tuple[bool, str]:
        ...
        tracked_proof, tracked_reason = self._tracked_bilateral_junction_proof(seam)
        measured_proof, measured_reason = self._measured_bilateral_junction_proof(seam)
        geometric_proof = bool(tracked_proof or measured_proof)
        proof_reason = measured_reason if measured_proof else tracked_reason
        remembered = self._remembered_end_near(s_j)
        if remembered is not None:
            # An end memory created before/just after lock acquisition is the
            # safety evidence that the lock itself sits on a physical end and
            # can never be overridden.  A memory born much later during a
            # mature, bilaterally supported junction track is a shortened-FOV
            # artifact; remove that late memory instead of emitting a veto
            # that a KLT coast would then bypass.
            start = self.junction_lock_start_frame
            birth = float(remembered[2]) if len(remembered) > 2 else 0.0
            late_memory = bool(
                geometric_proof
                and start is not None
                and birth > float(start)
                + float(self.params.get("junction_tracked_bilateral_late_memory_frames", 10))
            )
            if late_memory:
                for key, value in list(self._pipe_end_memory.items()):
                    if value is remembered:
                        del self._pipe_end_memory[key]
                        self._pipe_end_streak.pop(key, None)
                if measured_proof:
                    self._measured_bilateral_override_used = True
                    self._measured_bilateral_override_reason = (
                        f"late_memory_cleared:{proof_reason}"
                    )
                else:
                    self._tracked_axial_override_used = True
                    self._tracked_axial_override_reason = (
                        f"late_memory_cleared:{proof_reason}"
                    )
            else:
                if debug:
        ...
        if support_left_eff + 1e-6 < needed_left or support_right_eff + 1e-6 < needed_right:
            if geometric_proof:
                reason = (
                    f"short_extent:{proof_reason};"
                    f"L={support_left_eff:.3f}/{needed_left:.3f},"
                    f"R={support_right_eff:.3f}/{needed_right:.3f}"
                )
                if measured_proof:
                    self._measured_bilateral_override_used = True
                    self._measured_bilateral_override_reason = reason
                else:
                    self._tracked_axial_override_used = True
                    self._tracked_axial_override_reason = reason
                return True, ""
            if debug:
                uv = []
    ...
    def _localize_confirmed_seam(
        ...
        self._localize_base_model_used = False
        hold_override = bool(
            self.params.get("junction_station_output_override_in_hold", False)
            and self._pipe_state == "HOLD"
        )
        coast_override = bool(
            self.params.get("junction_station_output_override_when_coasted", False)
            and self._station_coast_used
            and self.junction_lock_source == "station_coasted"
        )
        override_mode = "hold" if hold_override else (
            "station_coast" if coast_override else None
        )
        if bool(self.params.get("junction_base_station_enabled", False)) and override_mode:
            # Measured root cause (2026-07-17): during the oblique sweep the
            # collar's rendered dark band is displaced ~5 mm from the projected
            # GT seam, so a HOLD-frame image measurement is metrically wrong
            # even when the 2D cue is tracked perfectly. A TRACK frame explicitly
            # accepted by station coast has the same requirement: projecting the
            # station back to a pixel and intersecting that ray reintroduces an
            # oblique-view axial error. Use the metric station directly in both
            # cases. Fail closed if station, support, FOV or pipe-end evidence is
            # unavailable.
            support_ready = bool(
                self._pipe_hold_mask_applied
                if override_mode == "hold"
                else self._station_coast_used
            )
            if (
                self.external_junction_station_active
                and self.external_junction_station_camera_xyz is not None
                and self._station_strip_x is not None
                and support_ready
            ):
                point = np.asarray(
                ...
                radius = None
                model = (
                    self.external_pipe_lock_camera_model
                    or self.pipe_component_selected_model
                    or self.pipe_lock_model
                )
                if model is not None:
                    comps = self._pipe_axis_model_components_from_model(model)
                    try:
                ...
                if (
                    ...
                    and point[2] > 0.02
                    and comps is not None
                    and radius is not None
                    and radius > 1e-4
                    and not self._pipe_end_rejected(
                        seam.strip_mask, int(round(self._station_strip_x))
                    )
                ):
                    # The gap-plane publisher requires a visible-surface point
                    # (its pose origin). For a station on the axis the surface
                    # the camera sees is analytic: one radius toward the
                    # camera, perpendicular to the axis.
                    axis, _ = comps
                    to_cam = -point
                    radial = to_cam - float(np.dot(to_cam, axis)) * axis
                    norm = float(np.linalg.norm(radial))
                    if norm > 1e-9:
                        surface = point + (radius / norm) * radial
                        self._station_override_used = True
                        self._station_override_reason = (
                            f"{override_mode}_station_age="
                            f"{float(self.external_junction_station_age_s):.2f}s"
                        )
                        # This is already the sensor-time projection of a fixed
                        # base-frame point. Camera-frame smoothing would lag it
                        # during arm motion, so reset the legacy filter exactly
                        # to the trusted metric state.
                        self._junction_center_state = point.copy()
                        self._junction_center_lag = np.zeros(3, dtype=np.float64)
                ...
                self._station_override_reason = "station_point_invalid_or_pipe_end"
            else:
                self._station_override_reason = (
                    f"station_unavailable_in_{override_mode}"
                )
        pipe_model = self.pipe_component_selected_model or self.pipe_lock_model
        if (
            bool(self.params.get("junction_localize_prefer_base_model", False))
            and pipe_model is not None
            and self.external_pipe_lock_camera_model is not None
        ):
            # A TRACK fit that deviates laterally from the TF-projected,
            # innovation-gated base cylinder beyond the base-lock gate is the
            # same fit the gate refuses to learn from; publishing weld geometry
            # from it while distrusting it is incoherent (measured: fresh
            # motion-TRACK poses with up to 10 mm vertical error while the
            # base projection stayed at 0.3 mm). Localize on the trusted
            # projection instead; the measured column still supplies the axial
            # coordinate.
            innovation = _pipe_model_innovation(
                pipe_model, self.external_pipe_lock_camera_model
            )
            if bool(innovation.get("valid")) and float(
                innovation.get("lateral_m", 0.0)
            ) > float(self.params.get("pipe_base_lock_max_lateral_innovation_m", 0.004)):
                pipe_model = dict(self.external_pipe_lock_camera_model)
                self._localize_base_model_used = True
    ...
    def _status_dict(
        ...
        pose_complete = self._localization_complete(localization)
        return {
            ...
            "junction_flow_x_unbounded_px": _round(
                self._junction_flow_x_unbounded
            ),
            "junction_single_frame_proof_used": bool(
                state_info.get("junction_single_frame_proof_used", False)
            ),
            "junction_single_frame_proof_reason": state_info.get(
                "junction_single_frame_proof_reason"
            ),
            "junction_hard_veto_output_blocked": bool(
                self._hard_veto_output_blocked
            ),
            "junction_out_of_view_side": self._junction_out_of_view_side,
            "junction_border_exit_coast_used": bool(self._border_exit_coast_used),
            "junction_border_exit_coast_reason": self._border_exit_coast_reason,
            "junction_border_prearm_used": bool(self._border_prearm_used),
            "junction_border_prearm_reason": self._border_prearm_reason,
            "junction_border_pipe_end_override_used": bool(
                self._border_pipe_end_override_used
            ),
            "junction_border_pipe_end_override_reason": (
                self._border_pipe_end_override_reason
            ),
            "junction_out_of_view_reentry_used": bool(
                self._out_of_view_reentry_used
            ),
            "junction_out_of_view_reentry_reason": self._out_of_view_reentry_reason,
            ...
            "variant_a_available": _variant_a_detect_seam is not None,
            ...
            "candidate_source": seam.candidate_source,
            "klt_border_candidate_used": bool(seam.klt_border_candidate_used),
            "klt_border_candidate_reason": seam.klt_border_candidate_reason,
            ...
            "pipe_hold_mask_applied": bool(self._pipe_hold_mask_applied),
            "pipe_hold_mask_reason": self._pipe_hold_mask_reason,
            "pipe_hold_cylinder_supported": bool(self._pipe_hold_cylinder_supported),
            "pipe_hold_model_source": self._pipe_hold_model_source,
            "pipe_hold_base_prediction_required": bool(
                self._pipe_hold_base_prediction_required
            ),
            "pipe_hold_base_prediction_available": bool(
                self._pipe_hold_base_prediction_available
            ),
            "pipe_base_projection_active": bool(self.external_pipe_lock_active),
            "pipe_base_projection_available": bool(
                self.external_pipe_lock_projection_available
            ),
            "pipe_base_projection_source": self.external_pipe_lock_projection_source,
            "pipe_hold_depth_support_checked": bool(self._pipe_hold_depth_support_checked),
            "pipe_hold_depth_support_passed": bool(self._pipe_hold_depth_support_passed),
            "pipe_hold_depth_support_med_mm": self._pipe_hold_depth_support_med_mm,
            "pipe_hold_depth_support_frac": self._pipe_hold_depth_support_frac,
            "pipe_partial_lock_support_used": bool(
                self._partial_pipe_support_used
            ),
            "pipe_partial_lock_support_reason": self._partial_pipe_support_reason,
            "junction_tracked_axial_override_used": bool(
                self._tracked_axial_override_used
            ),
            "junction_tracked_axial_override_reason": (
                self._tracked_axial_override_reason
            ),
            "junction_measured_bilateral_override_used": bool(
                self._measured_bilateral_override_used
            ),
            "junction_measured_bilateral_override_reason": (
                self._measured_bilateral_override_reason
            ),
            "junction_station_active": bool(self.external_junction_station_active),
            "junction_station_age_s": (
                None
                if not np.isfinite(float(self.external_junction_station_age_s))
                else float(self.external_junction_station_age_s)
            ),
            "junction_station_strip_x": self._station_strip_x,
            "junction_station_override_used": bool(self._station_override_used),
            "junction_localize_base_model_used": bool(
                getattr(self, "_localize_base_model_used", False)
            ),
            "junction_station_override_reason": self._station_override_reason,
            "junction_station_coast_used": bool(
                state_info.get("junction_station_coast_used", False)
            ),
            "junction_station_coast_reason": state_info.get(
                "junction_station_coast_reason"
            ),
            "junction_station_coast_frames": int(
                state_info.get("junction_station_coast_frames", 0)
            ),
            "junction_station_innovation_veto_used": bool(
                self._station_innovation_veto_used
            ),
            "junction_station_innovation_px": self._station_innovation_px,
            "junction_station_innovation_reason": self._station_innovation_reason,
            "pipe_trusted_axial_extent_m": (
                None
                if self._pipe_last_trusted_axial_extent is None
                else [_round(value, 4) for value in self._pipe_last_trusted_axial_extent]
            ),
            ...
            "junction_feature_width_px": int(seam.junction_feature_width_px),
            "junction_feature_shift_px": int(seam.junction_feature_shift_px),
            "junction_feature_centre_applied": bool(
                seam.junction_feature_centre_applied
            ),
            "junction_feature_centre_reason": seam.junction_feature_centre_reason,
            "junction_feature_anchor_distance_px": int(
                seam.junction_feature_anchor_distance_px
            ),
            ...
            # ``eligible`` describes the current 2D candidate. Controller-facing
            # acceptance additionally requires a complete current 3D pose. This
            # closes a measured CANDIDATE frame that was marked accepted even
            # though localization had produced neither centre nor surface.
            "detector_accepted": bool(
                state_info.get("eligible", False) and pose_complete
            ),
            "detector_pose_complete": bool(pose_complete),
            ...
            "measured_pipe_support_left_cols": int(
                seam.measured_pipe_support_left_cols
            ),
            "measured_pipe_support_right_cols": int(
                seam.measured_pipe_support_right_cols
            ),
            "measured_pipe_support_left_coverage": _round(
                seam.measured_pipe_support_left_coverage
            ),
            "measured_pipe_support_right_coverage": _round(
                seam.measured_pipe_support_right_coverage
            ),
            ...
            "pipe_axis_point_camera_xyz_m": _round_list(
                (self.pipe_component_selected_model or self.pipe_lock_model or {}).get("axis_point")
            ),
    ...
    @staticmethod
    def _localization_complete(
        localization: LocalizationResult | None,
    ) -> bool:
        if localization is None:
            return False
        for value in (
            localization.pipe_center_estimate_xyz_m,
            localization.visible_surface_center_xyz_m,
        ):
            if value is None:
                return False
            try:
    ...
    def _draw_overlay(
        ...
        line_uv = self._candidate_line_original_uv(seam)
        final_accepted = bool(
            state_info.get("eligible", False)
            and self._localization_complete(localization)
        )
        if final_accepted:
            color = (0, 220, 255, 255)
            label = "JUNCTION"
...
class AceaPipeJunctionNode(Node):
    def __init__(self) -> None:
        ...
        self._pipe_base_lock_model: dict[str, Any] | None = None
        self._pipe_base_lock_track_streak = 0
        self._pipe_base_lock_source = "none"
        self._pipe_base_lock_tf_query_source = "not_used"
        self._pipe_base_lock_tf_listener_mode = "not_used"
        self._pipe_base_lock_projection_available = False
        self._pipe_base_lock_last_update_accepted = False
        self._pipe_base_lock_last_reject_reason = "not_evaluated"
        self._pipe_base_lock_last_axis_innovation_deg: float | None = None
        self._pipe_base_lock_last_lateral_innovation_m: float | None = None
        self._pipe_base_lock_last_radius_innovation_m: float | None = None
        self._pipe_base_lock_rejected_update_count = 0
        self._pipe_base_lock_tf_cache_key: tuple[Any, ...] | None = None
        self._pipe_base_lock_tf_cache_value: tuple[np.ndarray, np.ndarray] | None = None
        self._pipe_base_lock_tf_cache_source = "not_cached"
        self._pipe_base_lock_generation = 0
        self._last_pipe_tracker_state = "ACQUIRE"
        # V14 station lifecycle. A provisional point must survive several
        # compatible fresh TRACK measurements before it becomes controller-
        # visible. The confirmed station is bound to one base-lock generation.
        self._junction_station_base_point: np.ndarray | None = None
        self._junction_station_stamp_s: float | None = None
        self._junction_station_generation: int | None = None
        self._junction_station_candidate_point: np.ndarray | None = None
        self._junction_station_candidate_stamp_s: float | None = None
        self._junction_station_candidate_streak = 0
        self._junction_station_update_count = 0
        self._junction_station_reject_count = 0
        self._junction_station_reset_count = 0
        self._junction_station_last_reset_reason = "never"
        self._junction_station_last_reject_reason = "not_evaluated"
        self.tf_buffer = None
        self.tf_listener = None
        self._pipe_tf_listener_node = None
        if bool(self.params.get("enable_pipe_base_link_lock", False)) and tf2_ros is not None:
            try:
                self.tf_buffer = tf2_ros.Buffer(node=self)
            except TypeError:
                self.tf_buffer = tf2_ros.Buffer()
            if bool(self.params.get("pipe_base_lock_dedicated_tf_listener", False)):
                try:
                    self._pipe_tf_listener_node = rclpy.create_node(
                        f"acea_pipe_junction_tf_listener_{os.getpid()}",
                    ...
                    self.tf_listener = tf2_ros.TransformListener(
                        self.tf_buffer,
                        self._pipe_tf_listener_node,
                        spin_thread=True,
                    )
                    self._pipe_base_lock_tf_listener_mode = "dedicated_node_thread"
                except Exception as exc:
                    if self._pipe_tf_listener_node is not None:
                        try:
                            self._pipe_tf_listener_node.destroy_node()
                        except Exception:
                            pass
                        self._pipe_tf_listener_node = None
                    self.get_logger().warning(
                        "dedicated TF listener setup failed; falling back to the detector executor: "
                        f"{type(exc).__name__}: {exc}"
                    )
                    self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)
                    self._pipe_base_lock_tf_listener_mode = "detector_executor_fallback"
            else:
                self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)
                self._pipe_base_lock_tf_listener_mode = "detector_executor"
        ...
        self.visual_pipe_mask_pub = None
        self.cylinder_inlier_mask_pub = None
        self.rejected_outliers_mask_pub = None
        if bool(self.params.get("publish_pipe_debug_masks", False)):
            self.visual_pipe_mask_pub = self.create_publisher(
                Image, str(self.params["visual_pipe_mask_topic"]), 10
            )
            self.cylinder_inlier_mask_pub = self.create_publisher(
                Image, str(self.params["cylinder_inlier_mask_topic"]), 10
            )
            self.rejected_outliers_mask_pub = self.create_publisher(
                Image, str(self.params["rejected_outliers_mask_topic"]), 10
            )
        ...
        if (
            str(self.params["junction_acceptance_mode"]).strip().lower()
            == "variant_a_rgb"
            and _variant_a_detect_seam is None
        ):
            self.get_logger().error(
                "Variant-A seam frontend is unavailable: acea_seam_detector_v15.py "
                "could not be imported. Junction detection will use only the "
                "fallback cue and is not valid for production."
            )
    ...
    def _declare_params(self) -> dict[str, Any]:
        declarations = {
            ...
            "weld_seam_use_axis_center_for_radial": False,
            # V15-only pose-origin normalization. Default OFF keeps every
            # frozen baseline and every non-V15 preset byte-for-byte in its
            # historical surface-point convention.
            "weld_surface_canonicalization_enabled": False,
            ...
            "min_depth_m": 0.05,
            "max_depth_m": 20.0,
            "pipe_foreground_mode": "kmeans_near",
            "sample_stride": 4,
            ...
            "pipe_mask_reproject_reuse_cached_normal_axis": False,
            ...
            # V10's color-off FSM can be made the sole owner of HOLD -> LOST.
            # It is opt-in so the validated real-camera baseline is unchanged.
            "pipe_fsm_owns_missed_release": False,
            ...
            # HOLD uses hysteresis: it is not allowed to change the cylinder,
            # but a partially visible trusted cylinder needs less evidence than
            # a global ACQUIRE candidate.  These values keep true close-up arcs
            # (align ~0.90, coverage 70-150deg) and reject the cloth-only outage
            # (negative slope and/or coverage below 50deg).
            "pipe_hold_min_normal_fraction": 0.85,
            "pipe_hold_min_cyl_slope": 0.04,
            "pipe_hold_min_cyl_align_frac": 0.75,
            "pipe_hold_min_cyl_cov_inlier_deg": 60.0,
            ...
            # Seam-centre cue. A dark feature wider than about half of
            # background_window_px defeats the local median, so the residual
            # peaks at the feature's edges and the seam is reported half a
            # feature-width off (measured: +/-9 mm on an 18 mm collar). Off by
            # default: the real preset must stay bit-identical until a
            # real-bag A/B covers it.
            "junction_feature_centre_enabled": False,
            ...
            # Local continuity gate while a junction lock is active.  Fresh
            # re-acquisition after an outage has its own expanding gate.
            "junction_lock_search_radius_px": 60,
            "junction_lock_local_visual_fallback_enabled": False,
            "junction_lock_local_visual_min_evidence_z": 3.0,
            # Disabled by default. A preset may require stronger evidence
            # before a large innovation updates an active lock; rejected
            # measurements still use the normal hold/coast path.
            "junction_lock_innovation_gate_px": 12.0,
            "junction_lock_innovation_min_evidence_z": 0.0,
            "junction_lock_respect_safety_veto": False,
            ...
            # Latest-TF lookup preserves the validated real-camera baseline.
            # A moving simulated camera enables sensor-stamped lookup in its
            # dedicated preset so the cylinder prediction and RGB-D frame use
            # the same camera pose.
            "pipe_base_lock_use_sensor_stamp_tf": False,
            # V12 safety controls are opt-in so loading the real-camera preset
            # through this development node still preserves v11 behaviour.
            "pipe_base_lock_tf_bounded_latest_fallback": False,
            "pipe_base_lock_tf_fallback_max_age_s": 0.04,
            "pipe_base_lock_innovation_gate_enabled": False,
            ...
            "pipe_hold_require_base_prediction": False,
            # ---- V13 (all default OFF; enabled only by development presets) --
            # HOLD support alternative for the close-view regime where the
            # azimuthal-slope discriminator is blind (visible azimuth span
            # collapses): accept the base-projected held mask when the measured
            # depth lies ON the predicted cylinder. Only valid for
            # base_link_prediction models (TF-projected + innovation-gated);
            # never for a stale camera-frame lock. Empirical residual against
            # the true cylinder at the failing stamps: median 3.5-3.9 mm.
            "pipe_hold_depth_support_enabled": False,
            "pipe_hold_depth_support_max_med_m": 0.006,
            "pipe_hold_depth_support_min_frac": 0.60,
            "pipe_hold_depth_support_tol_m": 0.010,
            # V15 may apply the same current-depth proof to a camera-frame lock
            # for a short bounded interval.  Disabled by default so every
            # frozen baseline/preset retains its original policy.
            "pipe_hold_depth_support_allow_camera_lock": False,
            "pipe_hold_camera_lock_max_frames": 6,
            # Count HOLD frames toward LOST release only when the held mask is
            # unsupported by the current frame. A verifiably present pipe
            # (depth+normals on the projected cylinder) keeps the lock alive;
            # persistent lack of support still releases after release_frames.
            "pipe_hold_release_only_when_unsupported": False,
            # Junction axial-station memory on the base cylinder. The station
            # is pinned/updated only from accepted fresh TRACK detections with
            # a bounded along-axis innovation, and during pipe HOLD the
            # published centre comes from the projected station instead of the
            # obliquely displaced image measurement.
            "junction_base_station_enabled": False,
            "junction_station_max_update_m": 0.003,
            "junction_station_update_gain": 0.3,
            "junction_station_max_age_s": 30.0,
            "junction_station_use_for_prediction": False,
            "junction_station_output_override_in_hold": False,
            "junction_localize_prefer_base_model": False,
            # ---- V14 lifecycle + bounded metric coast (all default OFF) -----
            # A single accepted pixel must never initialize a persistent weld
            # station. Confirmation requires consecutive, mutually compatible
            # fresh TRACK measurements on one base-lock generation.
            "junction_station_confirm_frames": 5,
            "junction_station_confirm_max_spread_m": 0.003,
            "pipe_base_lock_clear_on_lost": False,
            # Bridge only short visual dropouts with the confirmed station.
            # This does not consume the rejected image candidate and cannot
            # bypass current cylinder support, FOV or pipe-end vetoes.
            "junction_station_coast_enabled": False,
            "junction_station_coast_max_frames": 3,
            "junction_station_coast_edge_margin_px": 8,
            "junction_station_coast_require_pipe_support": True,
            # A confirmed metric station is stronger evidence than a later
            # image extremum. A visually strong but spatially incompatible
            # candidate is vetoed and may only use the bounded, independently
            # guarded station-coast path. Defaults remain off for real runs.
            "junction_station_strict_innovation_gate_enabled": False,
            "junction_station_max_innovation_px": 12.0,
            "junction_station_output_override_when_coasted": False,
            # ---- V15 color-OFF continuity (all default OFF) ----------------
            # Current-frame partial-cylinder proof. It does not relax global
            # RANSAC acquisition; it only preserves a compatible established
            # lock when the visible azimuth becomes too narrow for the normal-
            # rotation slope statistic.
            "pipe_partial_lock_support_enabled": False,
            "pipe_partial_lock_min_normal_fraction": 0.70,
            "pipe_partial_lock_max_axis_delta_deg": 3.0,
            "pipe_partial_lock_max_line_delta_m": 0.012,
            # Optional camera-motion allowance.  The base limit remains final
            # when there is no independently measured KLT displacement; only
            # the V15 color-OFF preset enables this bounded prediction.
            "pipe_partial_lock_motion_comp_enabled": False,
            "pipe_partial_lock_lateral_per_klt_px_m": 0.0017,
            "pipe_partial_lock_motion_max_extra_m": 0.040,
            "pipe_partial_lock_allow_current_selected": False,
            "pipe_partial_lock_max_radius_delta_m": 0.008,
            "pipe_partial_lock_max_residual_m": 0.008,
            "pipe_partial_lock_min_inlier_fraction": 0.60,
            # Single-frame startup proof: conjunction of a current valid
            # cylinder and a strong thin dark junction. It bypasses only the
            # temporal confirmation delay, never geometry/end vetoes.
            "junction_single_frame_proof_enabled": False,
            "junction_single_frame_min_normal_fraction": 0.70,
            "junction_single_frame_min_contrast": 0.05,
            "junction_single_frame_min_z": 4.0,
            "junction_single_frame_min_dark_score": 0.80,
            # Lock-aware local arbitration and an unsaturated flow coordinate.
            "junction_mature_lock_min_age_frames": 8,
            "junction_klt_prefer_mature_lock_enabled": False,
            "junction_unbounded_flow_enabled": False,
            "junction_klt_border_search_enabled": False,
            "junction_klt_border_min_visible_side_coverage": 0.85,
            "junction_klt_border_max_innovation_px": 6.0,
            "junction_klt_border_min_contrast": 0.015,
            "junction_klt_border_min_z": 3.0,
            "junction_klt_border_min_step_abs": 0.015,
            "junction_klt_border_min_step_z": 3.0,
            # Border lifecycle: a proven junction may finish crossing the FOV
            # using current optical flow, but no local candidate can reattach
            # while the signed track is outside. Re-entry requires current
            # inward motion plus a fresh luminance step on the pipe.
            "junction_border_exit_continuity_enabled": False,
            "junction_border_exit_max_weak_frames": 2,
            "junction_border_exit_min_klt_points": 20,
            "junction_border_prearm_enabled": False,
            "junction_border_prearm_extra_px": 24.0,
            "junction_border_prearm_min_evidence_z": 3.0,
            "junction_border_transition_slack_px": 2.0,
            "junction_reentry_border_slack_px": 12,
            "junction_reentry_min_inward_dx_px": 2.0,
            "junction_reentry_min_step_abs": 0.020,
            "junction_reentry_min_step_z": 1.5,
            "junction_out_of_view_max_frames": 30,
            # Resolve false metric-extent vetoes only for a mature KLT-aligned,
            # bilaterally supported junction. Early end memories remain final.
            "junction_tracked_bilateral_override_enabled": False,
            "junction_tracked_bilateral_max_klt_innovation_px": 8.0,
            "junction_tracked_bilateral_min_coverage": 0.90,
            "junction_tracked_bilateral_late_memory_frames": 10,
            # Current-frame geometric proof for a junction whose fitted axial
            # extent is shortened by grazing depth. Supporting pixels must lie
            # on the measured outer cylinder on both sides of a current visual
            # feature; defaults OFF and is enabled only by the V15 preset.
            "junction_measured_bilateral_support_enabled": False,
            ...
            # V15 current-frame fallback for tiny border/close-view slivers.
            # It never publishes the previous pose: acceptance requires a new
            # RGB line, bilateral current depth and a local task-radius
            # cylinder fitted from current normals. Defaults stay OFF so every
            # frozen baseline and preset remains byte-for-byte behavioural.
            "junction_fullframe_continuity_enabled": False,
            "junction_fullframe_fresh_border_acquire_enabled": False,
            "junction_fullframe_search_radius_px": 70,
            "junction_fullframe_fresh_border_search_radius_px": 24,
            "junction_fullframe_fresh_border_max_distance_px": 20,
            "junction_fullframe_max_missed_frames": 30,
            "junction_fullframe_normal_line_max_innovation_px": 18.0,
            "junction_fullframe_normal_line_max_angle_delta_deg": 15.0,
            "junction_fullframe_normal_outlier_search_radius_px": 40,
            # V16: station-arbitrated outlier veto (default OFF; see
            # the arbitration comment at the blocking site).
            "junction_fullframe_outlier_station_arbitration_enabled": False,
            "junction_fullframe_normal_outlier_min_recovery_streak": 2,
            "junction_fullframe_normal_outlier_self_check_radius_px": 4,
            "junction_fullframe_normal_self_check_min_coverage": 0.30,
            "junction_fullframe_normal_incomplete_search_radius_px": 40,
            # Current global-candidate rescue. Defaults OFF and is enabled only
            # by the V15 simulation preset. A global line is merely an expanded
            # search hint after a stale local-lock outlier; publication still
            # requires the independent current RGB/depth/cylinder proof.
            "junction_fullframe_global_candidate_rescue_enabled": False,
            "junction_fullframe_global_candidate_search_radius_px": 10,
            "junction_fullframe_global_candidate_min_separation_px": 8,
            "junction_fullframe_global_candidate_min_contrast": 0.05,
            "junction_fullframe_global_candidate_min_z": 8.0,
            "junction_fullframe_global_candidate_min_line_px": 70.0,
            "junction_fullframe_global_candidate_max_sync_dt_s": 0.040,
            "junction_fullframe_min_strong_run_px": 4,
            "junction_fullframe_min_depth_same_count": 3,
            "junction_fullframe_dark_threshold": 0.025,
            "junction_fullframe_strong_threshold": 0.050,
            "junction_fullframe_local_depth_layer_tol_m": 0.040,
            "junction_fullframe_local_ransac_iterations": 220,
            "junction_fullframe_local_vote_line_tol_m": 0.015,
            "junction_fullframe_local_min_votes": 25,
            "junction_fullframe_local_min_radial_support": 30,
            "junction_fullframe_local_radial_support_tol_m": 0.020,
            "junction_fullframe_line_support_tol_m": 0.020,
            "junction_fullframe_line_min_side_points": 6,
            "junction_fullframe_line_min_side_fraction": 0.50,
            "junction_fullframe_line_max_side_median_m": 0.015,
            "junction_fullframe_local_max_image_axis_delta_deg": 18.0,
            # Independent all-points cylinder estimate. Defaults OFF and is
            # enabled only in V15: a metric output is suppressed when two
            # current-frame estimators disagree on an underconstrained sliver.
            "junction_fullframe_independent_model_check_enabled": False,
            ...
            "junction_fullframe_foreground_layer_guard_enabled": False,
            "junction_fullframe_max_foreground_range_excess_m": 0.050,
            # Independent corroboration from the main pipe tracker, but only
            # when that tracker validated a current-frame TRACK component.
            # This resolves a one-frame ambiguity of the local crop without
            # ever accepting a HOLD/lock prediction. A long bilateral seam is
            # required; tiny micro-slivers continue to fail closed.
            "junction_fullframe_trusted_current_model_enabled": False,
            ...
            # A dedicated listener node can keep the TF buffer current while
            # the detector's single executor is busy in the heavy RGB-D
            # callback. This is safer than multithreading the detector itself.
            "pipe_base_lock_dedicated_tf_listener": False,
            # Legacy experiment switch retained for compatibility.
            ...
            # The full OpenCV filter backend is faster but is not numerically
            # equivalent: on the old real bag it changed the temporal lock and
            # produced a 73.6 mm centre error. Keep the geometry-equivalent
            # SciPy path as the V15 default; experimental backends remain
            # selectable explicitly for profiling only.
    ...
    def _try_process(self) -> None:
        ...
        try:
            ...
            depth_nonfinite_pixels = int(depth.size - np.count_nonzero(np.isfinite(depth)))
            if depth_nonfinite_pixels:
                # Gazebo represents empty background with +Inf.  Downstream
                # geometry already treats zero as invalid depth; normalize at
                # the ROS boundary so Inf never leaks into fitting/profiles.
                depth = np.nan_to_num(
                    depth,
                    copy=False,
                    nan=0.0,
            ...
            projected_base_lock = self._project_base_pipe_lock_to_camera(
                rgb_msg.header.frame_id,
                rgb_msg.header.stamp,
            )
            self._pipe_base_lock_projection_available = projected_base_lock is not None
            self.detector.external_pipe_lock_camera_model = projected_base_lock
            self.detector.external_pipe_lock_active = self._pipe_base_lock_model is not None
            self.detector.external_pipe_lock_projection_available = (
                projected_base_lock is not None
            )
            self.detector.external_pipe_lock_source = self._pipe_base_lock_source
            self.detector.external_pipe_lock_projection_source = (
                self._pipe_base_lock_tf_query_source
            )
            self._project_junction_station_to_camera(rgb_msg)
            self.detector.external_sync_time_source = (
                "receive" if self._effective_use_receive_time else "header"
            )
            self.detector.external_rgb_depth_dt_s = float(depth_time - rgb_time)
            ...
            status.update(
                {
                    ...
                    "depth_nonfinite_pixels": depth_nonfinite_pixels,
                }
            )
    ...
    def _lookup_base_from_camera(
        self,
        camera_frame: str,
        sensor_stamp: Any | None = None,
    ) -> tuple[np.ndarray, np.ndarray] | None:
        if (
            not bool(self.params.get("enable_pipe_base_link_lock", False))
            or self.tf_buffer is None
            or not camera_frame
            or Time is None
            or Duration is None
        ):
            self._pipe_base_lock_tf_query_source = "disabled_or_unavailable"
            return None
        target = str(self.params.get("pipe_base_lock_target_frame", "base_link"))
        use_sensor_stamp = bool(
            self.params.get("pipe_base_lock_use_sensor_stamp_tf", False)
        )
        query_time = Time()
        cache_key: tuple[Any, ...] | None = None
        if use_sensor_stamp and sensor_stamp is not None:
            try:
                query_time = Time.from_msg(sensor_stamp)
                self._pipe_base_lock_tf_query_source = "sensor_stamp"
                cache_key = (
                    target,
                    camera_frame,
                    int(sensor_stamp.sec),
                    int(sensor_stamp.nanosec),
                )
            except Exception:
                self._pipe_base_lock_tf_query_source = "invalid_sensor_stamp"
                return None
        else:
            self._pipe_base_lock_tf_query_source = "latest"
        if (
            cache_key is not None
            and cache_key == self._pipe_base_lock_tf_cache_key
            and self._pipe_base_lock_tf_cache_value is not None
        ):
            self._pipe_base_lock_tf_query_source = (
                f"{self._pipe_base_lock_tf_cache_source}:cache"
            )
            return self._pipe_base_lock_tf_cache_value
        try:
            ...
            exact_source = self._pipe_base_lock_tf_query_source
            fallback_enabled = bool(
                use_sensor_stamp
                and sensor_stamp is not None
                and self.params.get(
                    "pipe_base_lock_tf_bounded_latest_fallback", False
                )
            )
            if not fallback_enabled:
                self._pipe_base_lock_tf_query_source = f"{exact_source}:lookup_failed"
                return None
            try:
                tf = self.tf_buffer.lookup_transform(
                    target,
                ...
                sensor_s = float(sensor_stamp.sec) + 1e-9 * float(sensor_stamp.nanosec)
                tf_s = float(tf.header.stamp.sec) + 1e-9 * float(tf.header.stamp.nanosec)
                delta_s = abs(tf_s - sensor_s)
                max_age_s = float(
                    self.params.get("pipe_base_lock_tf_fallback_max_age_s", 0.04)
                )
                if not np.isfinite(delta_s) or tf_s <= 0.0 or delta_s > max_age_s:
                    self._pipe_base_lock_tf_query_source = (
                        f"{exact_source}:bounded_latest_rejected:dt={delta_s:.6f}"
                    )
                    return None
                self._pipe_base_lock_tf_query_source = (
                    f"{exact_source}:bounded_latest:dt={delta_s:.6f}"
                )
            except Exception:
                self._pipe_base_lock_tf_query_source = (
                    f"{exact_source}:lookup_failed:bounded_latest_failed"
                )
                return None
        tr = tf.transform.translation
        rot = tf.transform.rotation
        R_bc = _quat_xyzw_to_R([rot.x, rot.y, rot.z, rot.w])
        t_bc = np.array([tr.x, tr.y, tr.z], dtype=np.float64)
        if cache_key is not None:
            self._pipe_base_lock_tf_cache_key = cache_key
            self._pipe_base_lock_tf_cache_value = (R_bc, t_bc)
            self._pipe_base_lock_tf_cache_source = self._pipe_base_lock_tf_query_source
        return R_bc, t_bc

    def _project_base_pipe_lock_to_camera(
        self,
        camera_frame: str,
        sensor_stamp: Any | None = None,
    ) -> dict[str, Any] | None:
        if self._pipe_base_lock_model is None:
            return None
        resolved = self._lookup_base_from_camera(camera_frame, sensor_stamp)
        if resolved is None:
            return None
        R_bc, t_bc = resolved
        R_cb = R_bc.T
        base = self._pipe_base_lock_model
        try:
            axis_b = np.asarray(base["axis"], dtype=np.float64).reshape(3)
            point_b = np.asarray(base["axis_point"], dtype=np.float64).reshape(3)
        except Exception:
            return None
        model = dict(base)
        model["axis"] = _normalize(R_cb @ axis_b)
        model["axis_point"] = R_cb @ (point_b - t_bc)
        model["stand_off_m"] = float(model["axis_point"][2])
        return model

    def _reset_station_candidate(self) -> None:
        self._junction_station_candidate_point = None
        self._junction_station_candidate_stamp_s = None
        self._junction_station_candidate_streak = 0

    def _invalidate_junction_station(self, reason: str) -> None:
        had_state = bool(
            self._junction_station_base_point is not None
            or self._junction_station_candidate_point is not None
        )
        self._junction_station_base_point = None
        self._junction_station_stamp_s = None
        self._junction_station_generation = None
        self._reset_station_candidate()
        self.detector.external_junction_station_camera_xyz = None
        self.detector.external_junction_station_active = False
        self.detector.external_junction_station_age_s = float("inf")
        if had_state:
            self._junction_station_reset_count += 1
        self._junction_station_last_reset_reason = str(reason)

    def _clear_base_pipe_lock(self, reason: str) -> None:
        if self._pipe_base_lock_model is not None:
            self._pipe_base_lock_generation += 1
        self._pipe_base_lock_model = None
        self._pipe_base_lock_track_streak = 0
        self._pipe_base_lock_source = f"cleared:{reason}"
        self._pipe_base_lock_projection_available = False
        self._pipe_base_lock_tf_cache_key = None
        self._pipe_base_lock_tf_cache_value = None
        self._pipe_base_lock_tf_cache_source = "cleared"

    def _handle_station_lifecycle(self, status: dict[str, Any]) -> None:
        state = str(status.get("pipe_tracker_state") or "UNKNOWN")
        entered_untrusted = state in ("ACQUIRE", "LOST") and state != self._last_pipe_tracker_state
        if entered_untrusted:
            self._invalidate_junction_station(f"pipe_tracker_{state.lower()}")
            if bool(self.params.get("pipe_base_lock_clear_on_lost", False)):
                self._clear_base_pipe_lock(f"pipe_tracker_{state.lower()}")
        self._last_pipe_tracker_state = state

    def _project_junction_station_to_camera(self, rgb_msg: Image) -> None:
        """Hand the detector the base-frame junction station transformed to
        camera coordinates at this frame's RGB stamp, plus its age. Fail
        closed: any missing piece deactivates the station for this frame."""
        det = self.detector
        det.external_junction_station_camera_xyz = None
        det.external_junction_station_active = False
        det.external_junction_station_age_s = float("inf")
        if not bool(self.params.get("junction_base_station_enabled", False)):
            return
        if self._junction_station_base_point is None or self._pipe_base_lock_model is None:
            return
        if self._junction_station_generation != self._pipe_base_lock_generation:
            self._invalidate_junction_station("base_lock_generation_changed")
            return
        stamp = rgb_msg.header.stamp
        rgb_s = float(stamp.sec) + 1e-9 * float(stamp.nanosec)
        if self._junction_station_stamp_s is None:
            return
        age = rgb_s - float(self._junction_station_stamp_s)
        if not np.isfinite(age) or age < -0.5:
            return
        if age > float(self.params.get("junction_station_max_age_s", 30.0)):
            return
        resolved = self._lookup_base_from_camera(rgb_msg.header.frame_id, stamp)
        if resolved is None:
            return
        R_bc, t_bc = resolved
        # Project the ABSOLUTE base point onto the CURRENT base axis: the base
        # lock's axis_point drifts freely along the axis (deliberately
        # ungated), so a station stored relative to it would drift too.
        axis_b = _normalize(
            np.asarray(self._pipe_base_lock_model["axis"], dtype=np.float64).reshape(3)
        )
        point_b = np.asarray(
            self._pipe_base_lock_model["axis_point"], dtype=np.float64
        ).reshape(3)
        station_b = np.asarray(self._junction_station_base_point, dtype=np.float64).reshape(3)
        s = float(np.dot(station_b - point_b, axis_b))
        on_axis_b = point_b + s * axis_b
        cam_point = R_bc.T @ (on_axis_b - t_bc)
        if not np.isfinite(cam_point).all():
            return
        det.external_junction_station_camera_xyz = cam_point
        det.external_junction_station_age_s = max(0.0, float(age))
        det.external_junction_station_active = True

    def _update_junction_station(
        self,
        status: dict[str, Any],
        camera_frame: str,
        sensor_stamp: Any | None,
    ) -> None:
        """Confirm and update one station on one persistent-cylinder generation.

        Initialization needs several consecutive compatible fresh TRACK
        detections. HOLD/coasted/rejected samples never initialize or update the
        station. LOST/ACQUIRE and base-lock generation changes invalidate it.
        """
        self._junction_station_last_reject_reason = "not_evaluated"
        if not bool(self.params.get("junction_base_station_enabled", False)):
            self._junction_station_last_reject_reason = "disabled"
            return
        if self._pipe_base_lock_model is None:
            if self._junction_station_base_point is None:
                self._reset_station_candidate()
            self._junction_station_last_reject_reason = "no_base_lock"
            return
        if not bool(status.get("detector_accepted", False)):
            if self._junction_station_base_point is None:
                self._reset_station_candidate()
            self._junction_station_last_reject_reason = "not_accepted"
            return
        if str(status.get("pipe_tracker_state")) != "TRACK":
            if self._junction_station_base_point is None:
                self._reset_station_candidate()
            self._junction_station_last_reject_reason = "tracker_not_track"
            return
        if str(status.get("junction_lock_source")) != "fresh_detection":
            if self._junction_station_base_point is None:
                self._reset_station_candidate()
            self._junction_station_last_reject_reason = "not_fresh_detection"
            return
        centre_c = self._status_vector3(status, "coarse_seam_center_camera_xyz_m")
        if centre_c is None:
            self._junction_station_last_reject_reason = "no_seam_centre"
            return
        if sensor_stamp is None:
            self._junction_station_last_reject_reason = "no_sensor_stamp"
            return
        resolved = self._lookup_base_from_camera(camera_frame, sensor_stamp)
        if resolved is None:
            self._junction_station_last_reject_reason = "tf_unavailable"
            return
        R_bc, t_bc = resolved
        centre_b = R_bc @ centre_c + t_bc
        axis_b = _normalize(
            np.asarray(self._pipe_base_lock_model["axis"], dtype=np.float64).reshape(3)
        )
        point_b = np.asarray(
        ...
        s = float(np.dot(centre_b - point_b, axis_b))
        measured_station = point_b + s * axis_b
        stamp_s = float(sensor_stamp.sec) + 1e-9 * float(sensor_stamp.nanosec)
        if self._junction_station_base_point is None:
            max_spread = float(
                self.params.get("junction_station_confirm_max_spread_m", 0.003)
            )
            candidate = self._junction_station_candidate_point
            if candidate is None:
                self._junction_station_candidate_point = measured_station
                self._junction_station_candidate_streak = 1
            else:
                candidate = np.asarray(candidate, dtype=np.float64).reshape(3)
                delta = abs(float(np.dot(measured_station - candidate, axis_b)))
                if delta > max_spread:
                    self._junction_station_reject_count += 1
                    self._junction_station_candidate_point = measured_station
                    self._junction_station_candidate_streak = 1
                    self._junction_station_last_reject_reason = (
                        f"candidate_reset_delta={delta*1000.0:.1f}mm"
                    )
                    self._junction_station_candidate_stamp_s = stamp_s
                    return
                streak = int(self._junction_station_candidate_streak)
                self._junction_station_candidate_point = (
                    float(streak) * candidate + measured_station
                ) / float(streak + 1)
                self._junction_station_candidate_streak = streak + 1
            self._junction_station_candidate_stamp_s = stamp_s
            required = max(
                2, int(self.params.get("junction_station_confirm_frames", 5))
            )
            if self._junction_station_candidate_streak < required:
                self._junction_station_last_reject_reason = (
                    f"confirming:{self._junction_station_candidate_streak}/{required}"
                )
                return
            self._junction_station_base_point = np.asarray(
                self._junction_station_candidate_point, dtype=np.float64
            ).reshape(3)
            self._junction_station_stamp_s = stamp_s
            self._junction_station_generation = int(self._pipe_base_lock_generation)
            self._junction_station_update_count += 1
            self._junction_station_last_reject_reason = "accepted:confirmed"
            self._reset_station_candidate()
            return
        if self._junction_station_generation != self._pipe_base_lock_generation:
            self._invalidate_junction_station("base_lock_generation_changed")
            self._junction_station_last_reject_reason = "generation_changed"
            return
        prev = np.asarray(self._junction_station_base_point, dtype=np.float64).reshape(3)
        delta_along = abs(float(np.dot(measured_station - prev, axis_b)))
        if delta_along > float(self.params.get("junction_station_max_update_m", 0.003)):
            self._junction_station_reject_count += 1
            self._junction_station_last_reject_reason = (
                f"delta_along={delta_along*1000.0:.1f}mm"
            )
            return
        gain = float(self.params.get("junction_station_update_gain", 0.3))
        self._junction_station_base_point = (1.0 - gain) * prev + gain * measured_station
        self._junction_station_stamp_s = stamp_s
        self._junction_station_update_count += 1
        self._junction_station_last_reject_reason = "accepted:updated"

    def _junction_station_status(self) -> dict[str, Any]:
        point = self._junction_station_base_point
        return {
            "junction_station_enabled": bool(
                self.params.get("junction_base_station_enabled", False)
            ),
            "junction_station_base_point_xyz_m": (
                None if point is None else [round(float(v), 6) for v in point]
            ),
            "junction_station_update_count": int(self._junction_station_update_count),
            "junction_station_reject_count": int(self._junction_station_reject_count),
            "junction_station_confirm_streak": int(
                self._junction_station_candidate_streak
            ),
            "junction_station_confirm_required": max(
                2, int(self.params.get("junction_station_confirm_frames", 5))
            ),
            "junction_station_generation": self._junction_station_generation,
            "pipe_base_lock_generation": int(self._pipe_base_lock_generation),
            "junction_station_reset_count": int(self._junction_station_reset_count),
            "junction_station_reset_reason": self._junction_station_last_reset_reason,
            "junction_station_update_reason": self._junction_station_last_reject_reason,
        }

    def _update_base_pipe_lock(
        self,
        status: dict[str, Any],
        camera_frame: str,
        sensor_stamp: Any | None = None,
        ...
        self._pipe_base_lock_last_update_accepted = False
        self._pipe_base_lock_last_reject_reason = "not_evaluated"
        self._pipe_base_lock_last_axis_innovation_deg = None
        self._pipe_base_lock_last_lateral_innovation_m = None
        self._pipe_base_lock_last_radius_innovation_m = None
        if not bool(self.params.get("enable_pipe_base_link_lock", False)):
            self._pipe_base_lock_last_reject_reason = "disabled"
            return
        if str(status.get("pipe_tracker_state")) != "TRACK":
            self._pipe_base_lock_track_streak = 0
            self._pipe_base_lock_last_reject_reason = "tracker_not_track"
            return
        axis_c = self._status_vector3(status, "pipe_axis_camera_xyz")
        point_c = self._status_vector3(status, "pipe_axis_point_camera_xyz_m")
        radius = status.get("pipe_pose_radius_m")
        residual = status.get("pipe_pose_residual_m")
        inlier_fraction = status.get("pipe_pose_inlier_fraction")
        if axis_c is None or point_c is None or radius is None:
            self._pipe_base_lock_track_streak = 0
            self._pipe_base_lock_last_reject_reason = "invalid_camera_model"
            return
        resolved = self._lookup_base_from_camera(camera_frame, sensor_stamp)
        if resolved is None:
            self._pipe_base_lock_source = "tf_unavailable"
            self._pipe_base_lock_last_reject_reason = "tf_unavailable"
            return
        self._pipe_base_lock_track_streak += 1
        min_frames = int(self.params.get("pipe_base_lock_min_track_frames", 8))
        if self._pipe_base_lock_track_streak < min_frames:
            self._pipe_base_lock_source = "warming_up"
            self._pipe_base_lock_last_reject_reason = "warming_up"
            return
        R_bc, t_bc = resolved
        axis_b_meas = _normalize(R_bc @ axis_c)
        point_b_meas = R_bc @ point_c + t_bc
        new_model = {
            "axis": axis_b_meas,
            "axis_point": point_b_meas,
            "radius_m": float(radius),
            "stand_off_m": float(point_b_meas[2]),
            "residual_m": float(residual) if residual is not None else 0.0,
            "inlier_fraction": float(inlier_fraction) if inlier_fraction is not None else 1.0,
        }
        if self._pipe_base_lock_model is None:
            self._pipe_base_lock_generation += 1
            self._invalidate_junction_station("base_lock_initialized")
            self._pipe_base_lock_model = new_model
            self._pipe_base_lock_source = "initialized"
            self._pipe_base_lock_last_update_accepted = True
            self._pipe_base_lock_last_reject_reason = "accepted:initialized"
            return
        gain = float(self.params.get("pipe_base_lock_correction_gain", 0.08))
        innovation = _pipe_model_innovation(self._pipe_base_lock_model, new_model)
        if bool(innovation["valid"]):
            self._pipe_base_lock_last_axis_innovation_deg = float(innovation["axis_deg"])
            self._pipe_base_lock_last_lateral_innovation_m = float(innovation["lateral_m"])
            self._pipe_base_lock_last_radius_innovation_m = float(innovation["radius_m"])
        gate_reason = None
        if not bool(innovation["valid"]):
            gate_reason = "invalid_innovation"
        elif bool(self.params.get("pipe_base_lock_innovation_gate_enabled", False)):
            if float(innovation["axis_deg"]) > float(
                self.params.get("pipe_base_lock_max_axis_innovation_deg", 3.0)
            ):
                gate_reason = "axis_innovation"
            elif float(innovation["lateral_m"]) > float(
                self.params.get("pipe_base_lock_max_lateral_innovation_m", 0.004)
            ):
                gate_reason = "lateral_innovation"
            elif float(innovation["radius_m"]) > float(
                self.params.get("pipe_base_lock_max_radius_innovation_m", 0.01)
            ):
                gate_reason = "radius_innovation"
        if gate_reason is not None:
            self._pipe_base_lock_rejected_update_count += 1
            self._pipe_base_lock_source = f"held:{gate_reason}"
            self._pipe_base_lock_last_reject_reason = gate_reason
            return
        prev_axis = _normalize(
            np.asarray(self._pipe_base_lock_model["axis"], dtype=np.float64).reshape(3)
        )
        axis_b_meas = np.asarray(innovation["axis_aligned"], dtype=np.float64)
        self._pipe_base_lock_model = {
            **new_model,
            "axis": _normalize((1.0 - gain) * prev_axis + gain * axis_b_meas),
            "axis_point": (
                (1.0 - gain) * np.asarray(self._pipe_base_lock_model["axis_point"], dtype=np.float64).reshape(3)
                + gain * point_b_meas
            ),
        }
        self._pipe_base_lock_source = "corrected"
        self._pipe_base_lock_last_update_accepted = True
        self._pipe_base_lock_last_reject_reason = "accepted:corrected"

    def _base_pipe_lock_status(self) -> dict[str, Any]:
        model = self._pipe_base_lock_model
        min_frames = max(1, int(self.params.get("pipe_base_lock_min_track_frames", 8)))
        confidence = 0.0 if model is None else min(1.0, float(self._pipe_base_lock_track_streak) / float(min_frames))
        return {
            "pipe_base_lock_enabled": bool(self.params.get("enable_pipe_base_link_lock", False)),
            "pipe_base_lock_active": model is not None,
            "pipe_base_lock_source": self._pipe_base_lock_source,
            "pipe_base_lock_track_streak": int(self._pipe_base_lock_track_streak),
            "pipe_base_lock_target_frame": str(self.params.get("pipe_base_lock_target_frame", "base_link")),
            "pipe_base_lock_tf_query_source": self._pipe_base_lock_tf_query_source,
            "pipe_base_lock_tf_listener_mode": self._pipe_base_lock_tf_listener_mode,
            "pipe_base_lock_projection_available": bool(
                self._pipe_base_lock_projection_available
            ),
            "pipe_base_lock_last_update_accepted": bool(
                self._pipe_base_lock_last_update_accepted
            ),
            "pipe_base_lock_last_reject_reason": self._pipe_base_lock_last_reject_reason,
            "pipe_base_lock_last_axis_innovation_deg": _round(
                self._pipe_base_lock_last_axis_innovation_deg, 4
            ),
            "pipe_base_lock_last_lateral_innovation_m": _round(
                self._pipe_base_lock_last_lateral_innovation_m, 6
            ),
            "pipe_base_lock_last_radius_innovation_m": _round(
                self._pipe_base_lock_last_radius_innovation_m, 6
            ),
            "pipe_base_lock_rejected_update_count": int(
                self._pipe_base_lock_rejected_update_count
            ),
            "pipe_axis_base_link_xyz": None if model is None else _round_list(model.get("axis")),
            "pipe_axis_point_base_link_xyz_m": None if model is None else _round_list(model.get("axis_point")),
            "pipe_radius_base_lock_m": None if model is None else _round(model.get("radius_m")),
            "pipe_base_lock_confidence": _round(confidence, 3),
        }
    ...
    def _build_weld_gap_geometry(
        ...
        use_axis_center_radial = bool(
            self.params["weld_seam_use_axis_center_for_radial"]
        )
        ...
        if require_accepted and not detector_accepted:
            ...
            plane_valid = False
        elif use_axis_center_radial and center_point is None:
            reason = "missing_coarse_seam_center_camera_xyz_m"
            plane_valid = False
        ...
        if plane_valid:
            ...
            if surface_point is None:
                ...
                seam_frame = seam_frame_from_axis_and_surface(
                    axis,
                    surface_point,
                    center_point if use_axis_center_radial else None,
                )
        ...
        payload = {
            ...
            "seam_frame_radial_source": (
                "axis_line_toward_camera"
                if use_axis_center_radial
                else "legacy_camera_origin_projection"
            ),
    ...
    def _publish_debug_status(self, status: dict[str, Any]) -> None:
        ...
        compact = {
            ...
            "pipe_axis_camera_xyz": status.get("pipe_axis_camera_xyz"),
            "pipe_axis_point_camera_xyz_m": status.get("pipe_axis_point_camera_xyz_m"),
            "pipe_tracker_state": status.get("pipe_tracker_state"),
            "pipe_hold_mask_applied": status.get("pipe_hold_mask_applied"),
            "pipe_hold_mask_reason": status.get("pipe_hold_mask_reason"),
            "pipe_hold_model_source": status.get("pipe_hold_model_source"),
            "pipe_hold_base_prediction_required": status.get(
                "pipe_hold_base_prediction_required"
            ),
            "pipe_hold_base_prediction_available": status.get(
                "pipe_hold_base_prediction_available"
            ),
            "pipe_base_projection_active": status.get("pipe_base_projection_active"),
            "pipe_base_projection_available": status.get(
                "pipe_base_projection_available"
            ),
            "pipe_base_projection_source": status.get("pipe_base_projection_source"),
            "pipe_base_lock_enabled": status.get("pipe_base_lock_enabled"),
            "pipe_base_lock_active": status.get("pipe_base_lock_active"),
            "pipe_base_lock_source": status.get("pipe_base_lock_source"),
            "pipe_base_lock_track_streak": status.get("pipe_base_lock_track_streak"),
            "pipe_base_lock_target_frame": status.get("pipe_base_lock_target_frame"),
            "pipe_base_lock_tf_query_source": status.get("pipe_base_lock_tf_query_source"),
            "pipe_base_lock_tf_listener_mode": status.get("pipe_base_lock_tf_listener_mode"),
            "pipe_base_lock_projection_available": status.get(
                "pipe_base_lock_projection_available"
            ),
            "pipe_base_lock_last_update_accepted": status.get(
                "pipe_base_lock_last_update_accepted"
            ),
            "pipe_base_lock_last_reject_reason": status.get(
                "pipe_base_lock_last_reject_reason"
            ),
            "pipe_base_lock_last_axis_innovation_deg": status.get(
                "pipe_base_lock_last_axis_innovation_deg"
            ),
            "pipe_base_lock_last_lateral_innovation_m": status.get(
                "pipe_base_lock_last_lateral_innovation_m"
            ),
            "pipe_base_lock_last_radius_innovation_m": status.get(
                "pipe_base_lock_last_radius_innovation_m"
            ),
            "pipe_base_lock_rejected_update_count": status.get(
                "pipe_base_lock_rejected_update_count"
            ),
            "pipe_axis_base_link_xyz": status.get("pipe_axis_base_link_xyz"),
            "pipe_axis_point_base_link_xyz_m": status.get("pipe_axis_point_base_link_xyz_m"),
            "pipe_radius_base_lock_m": status.get("pipe_radius_base_lock_m"),
            "pipe_base_lock_confidence": status.get("pipe_base_lock_confidence"),
    ...
    def _publish(self, status: dict[str, Any], rgb_overlay: np.ndarray, depth_overlay: np.ndarray, header: Any) -> None:
        ...
        camera_frame = "" if header is None else str(getattr(header, "frame_id", "") or "")
        sensor_stamp = None if header is None else getattr(header, "stamp", None)
        self._handle_station_lifecycle(status)
        self._update_base_pipe_lock(status, camera_frame, sensor_stamp)
        self._update_junction_station(status, camera_frame, sensor_stamp)
        status.update(self._base_pipe_lock_status())
        status.update(self._junction_station_status())
        ...
        self._publish_pipe_debug_masks(header)

    def _publish_pipe_debug_masks(self, header: Any) -> None:
        if not bool(self.params.get("publish_pipe_debug_masks", False)):
            return
        masks = (
            (self.visual_pipe_mask_pub, self.detector._debug_visual_pipe_mask),
            (self.cylinder_inlier_mask_pub, self.detector._debug_cylinder_inlier_mask),
            (self.rejected_outliers_mask_pub, self.detector._debug_rejected_outliers_mask),
        )
        for pub, mask in masks:
            if pub is None or mask is None:
                continue
            pub.publish(self._mask_to_msg(mask, header))
    ...
    @staticmethod
    def _mask_to_msg(mask: np.ndarray, header: Any) -> Image:
        arr = (np.asarray(mask, dtype=bool).astype(np.uint8) * 255)
        output = Image()
        output.header = header
        output.height = int(arr.shape[0])
        output.width = int(arr.shape[1])
        output.encoding = "mono8"
        output.is_bigendian = 0
        output.step = int(arr.shape[1])
        output.data = np.ascontiguousarray(arr).tobytes()
        return output

    def shutdown_auxiliary_nodes(self) -> None:
        """Stop the optional TF executor before destroying this detector node."""
...
def main(args: list[str] | None = None) -> None:
    ...
    try:
        ...
        if (
            bool(node.params.get("pipe_base_lock_multithreaded_executor", False))
