...
try:
    ...
    Duration = Time = tf2_ros = None  # type: ignore
...
class OnlinePipeJunctionDetector:
    ...
    def _update_pipe_debug_masks(
        ...
        visual = np.asarray(tracker.pipe_mask, dtype=bool)
        inlier = np.zeros_like(visual, dtype=bool)
        model = self.pipe_lock_model if self._pipe_tracker_hold_frame else self.pipe_component_selected_model
        if model is None:
            model = self.pipe_lock_model
        try:
            axis = _normalize(np.asarray(model["axis"], dtype=np.float64).reshape(3))
...
class AceaPipeJunctionNode(Node):
    ...
    def _try_process(self) -> None:
        ...
        try:
            ...
            self.detector.external_pipe_lock_camera_model = self._project_base_pipe_lock_to_camera(
    ...
    def _lookup_base_from_camera(self, camera_frame: str) -> tuple[np.ndarray, np.ndarray] | None:
        if (
    ...
    def _project_base_pipe_lock_to_camera(self, camera_frame: str) -> dict[str, Any] | None:
        if self._pipe_base_lock_model is None:
            return None
        resolved = self._lookup_base_from_camera(camera_frame)
        if resolved is None:
            return None
        R_bc, t_bc = resolved
        R_cb = R_bc.T
    ...
    def _update_base_pipe_lock(self, status: dict[str, Any], camera_frame: str) -> None:
        ...
        prev_axis = _normalize(np.asarray(self._pipe_base_lock_model["axis"], dtype=np.float64).reshape(3))
        if float(np.dot(axis_b_meas, prev_axis)) < 0.0:
            axis_b_meas = -axis_b_meas
    ...
    def _publish(self, status: dict[str, Any], rgb_overlay: np.ndarray, depth_overlay: np.ndarray, header: Any) -> None:
        ...
        self._update_base_pipe_lock(status, camera_frame)
