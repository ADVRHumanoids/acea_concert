...
try:
    import cv2
except ImportError:  # pragma: no cover - exercised by the SciPy fallback
    cv2 = None
...
DEFAULT_PARAMS: dict[str, Any] = {
    ...
    # V15 keeps the complete angular search but moves its repeated rotate,
    # morphology and vertical Gaussian operations to OpenCV float32. The
    # SciPy backend remains available for frame-identical A/B validation.
    "backend": "opencv",
}
...
def _detect_seam_on_gray_opencv(
    gray: np.ndarray,
    p: dict[str, Any],
    orientation_deg: float,
) -> SeamDetection:
    """OpenCV float32 equivalent of :func:`_detect_seam_on_gray`.

    The scalar scoring and all gates intentionally match the frozen SciPy
    implementation. Only the expensive horizontal closing and vertical
    Gaussian are delegated to OpenCV. This function is kept separate so the
    old backend can serve as a parity oracle during V15 validation.
    """
    if cv2 is None:
        return _detect_seam_on_gray(gray, p, orientation_deg)

    work = np.asarray(gray, dtype=np.float32)
    h, w = work.shape
    r0, r1 = _pipe_band(work, p["pipe_bright_frac"])

    footprint = np.ones((1, int(p["tophat_se_len_px"])), dtype=np.uint8)
    ...
    resp = np.maximum(closed - work, 0.0)

    sigma = float(p["vert_smooth_sigma_px"])
    if sigma > 0.0:
        # scipy.ndimage.gaussian_filter uses truncate=4 by default.
        kernel_size = int(2 * np.ceil(4.0 * sigma) + 1)
        kernel_y = cv2.getGaussianKernel(kernel_size, sigma, cv2.CV_32F)
        respv = cv2.sepFilter2D(
            resp,
            cv2.CV_32F,
    ...
    colresp = resp[:, max(0, x - nb):min(w, x + nb + 1)].mean(axis=1)
    cmax = float(colresp.max())
    threshold = p["resp_span_thresh_frac"] * cmax if cmax > 0 else np.inf
    on = colresp >= threshold
    run = run_best = 0
    for value in on:
    ...
    return SeamDetection(
        ...
        span_frac,
        "ok" if accepted else ";".join(reasons),
        vertical_run_px=vertical_run_px,
        pipe_band=(r0, r1),
        orientation_deg=float(orientation_deg),
        column_profile=col,
    )
...
def detect_seam(rgb: np.ndarray, params: dict | None = None,
    ...
    backend = str(p.get("backend", "opencv")).strip().lower()
    valid_backends = {"scipy", "opencv", "opencv_filters", "opencv_rotate"}
    if backend not in valid_backends:
        raise ValueError(
            f"unknown seam backend {backend!r}; expected one of {sorted(valid_backends)}"
        )
    use_opencv_rotate = backend in {"opencv", "opencv_rotate"} and cv2 is not None
    use_opencv_filters = backend in {"opencv", "opencv_filters"} and cv2 is not None
    gray0_cv = (
        np.asarray(gray0, dtype=np.float32)
        if use_opencv_rotate or use_opencv_filters
        else None
    )
    best: SeamDetection | None = None
    for angle in angles:
        if use_opencv_rotate and abs(angle) > 1.0:
            h, w = gray0_cv.shape
            ...
            gray = gray0_cv if use_opencv_rotate or use_opencv_filters else gray0
        candidate = (
            _detect_seam_on_gray_opencv(gray, p, angle)
            if use_opencv_filters
            else _detect_seam_on_gray(gray, p, angle)
        )
