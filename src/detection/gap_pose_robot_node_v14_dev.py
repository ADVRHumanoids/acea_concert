#!/usr/bin/env python3
"""
gap_pose_robot_node_v14_dev.py - V14 perception-driven pose bridge.
...
def _stamp_parts_or_none(value) -> tuple[int, int] | None:
    """Parse the detector's JSON timestamp without depending on ROS messages."""
...
def _selftest() -> int:
    ...
    checks.append((
        "valid detector stamp is parsed",
        _stamp_parts_or_none({"sec": 12, "nanosec": 34}) == (12, 34),
    ))
    checks.append((
        "invalid detector stamp is rejected",
        _stamp_parts_or_none({"sec": 12, "nanosec": 1_000_000_000}) is None,
    ))
...
def _build_node_class():
    ...
    class GapPoseRobotNode(Node):
        ...
        def __init__(self) -> None:
            ...
            self.use_sensor_stamp_tf = bool(p("use_sensor_stamp_tf", False).value)
            self.dedicated_tf_listener = bool(p("dedicated_tf_listener", False).value)
            self.tf_bounded_latest_fallback = bool(
                p("tf_bounded_latest_fallback", False).value
            )
            self.tf_fallback_max_age_s = float(
                p("tf_fallback_max_age_s", 0.04).value
            )
            self.preserve_input_stamp = bool(p("preserve_input_stamp", False).value)
            ...
            self.radial_orientation_mode = str(
                p("radial_orientation_mode", "measured").value
            ).strip().lower()
            if self.radial_orientation_mode not in (
                "measured",
                "base_to_axis",
                "horizontal_from_axis",
            ):
                raise ValueError(
                    "radial_orientation_mode must be 'measured' or "
                    "'base_to_axis' or 'horizontal_from_axis'"
                )
            ...
            self.status_topic = str(
                p("status_topic", "/acea/gap_pose_robot/status").value
            )
            ...
            self._last_tf_query_source = "not_used"
            self._last_tf_delta_s = None
            self._tf_listener_node = None
            try:
                self.tf_buffer = tf2_ros.Buffer(node=self)
            except TypeError:
                self.tf_buffer = tf2_ros.Buffer()
            if self.dedicated_tf_listener:
                try:
                    self._tf_listener_node = rclpy.create_node(
                        f"gap_pose_robot_tf_listener_{os.getpid()}",
                    ...
                    self.tf_listener = tf2_ros.TransformListener(
                        self.tf_buffer,
                        self._tf_listener_node,
                        spin_thread=True,
                    )
                    self._tf_listener_mode = "dedicated_node_thread"
                    ...
                    self.get_logger().warning(
                        "dedicated TF listener setup failed; using bridge executor: "
                        f"{type(exc).__name__}: {exc}"
                    )
                    self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)
                    self._tf_listener_mode = "bridge_executor_fallback"
            else:
                self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)
                self._tf_listener_mode = "bridge_executor"
            ...
            self.get_logger().info(
                ...
                f"sensor_stamp_tf={'on' if self.use_sensor_stamp_tf else 'off'}, "
                f"tf_listener={self._tf_listener_mode}, "
                f"preserve_input_stamp={'on' if self.preserve_input_stamp else 'off'}, "
                f"radial_mode={self.radial_orientation_mode}")
        ...
        def _publish_status(
            ...
            *,
            ...
            reason: str,
            camera_frame: str = "",
            input_stamp: tuple[int, int] | None = None,
        ) -> None:
            if not accepted:
            ...
            payload = {
                ...
                "reason": str(reason),
                "input_count": int(self._input_count),
                ...
                else {"sec": int(input_stamp[0]), "nanosec": int(input_stamp[1])},
                "input_stamp_s": None
                if input_stamp is None
                else float(input_stamp[0]) + 1e-9 * float(input_stamp[1]),
                "tf_query_source": self._last_tf_query_source,
                "tf_delta_s": self._last_tf_delta_s,
                "tf_listener_mode": self._tf_listener_mode,
            }
        ...
        def _resolve_transform(
            ...
            if self.use_tf and camera_frame:
                try:
                    ...
                    if self.use_sensor_stamp_tf:
                        if input_stamp is None:
                            raise ValueError("detector message has no valid RGB timestamp")
                        query_time = rclpy.time.Time(
                            seconds=input_stamp[0],
                            nanoseconds=input_stamp[1],
                            clock_type=self.get_clock().clock_type,
                        )
                        self._last_tf_query_source = "sensor_stamp"
                    else:
                        query_time = rclpy.time.Time()
                        self._last_tf_query_source = "latest"
                    ...
                    exact_source = self._last_tf_query_source
                    if (
                        self.use_sensor_stamp_tf
                        and input_stamp is not None
                        and self.tf_bounded_latest_fallback
                    ):
                        try:
                            tf = self.tf_buffer.lookup_transform(
                                self.target_frame,
                            ...
                            sensor_s = float(input_stamp[0]) + 1e-9 * float(input_stamp[1])
                            tf_s = float(tf.header.stamp.sec) + 1e-9 * float(
                                tf.header.stamp.nanosec
                            )
                            delta_s = abs(tf_s - sensor_s)
                            self._last_tf_delta_s = float(delta_s)
                            if (
                                not np.isfinite(delta_s)
                                or tf_s <= 0.0
                                or delta_s > max(0.0, self.tf_fallback_max_age_s)
                            ):
                                self._last_tf_query_source = (
                                    f"{exact_source}:bounded_latest_rejected"
                                )
                            else:
                                tr = tf.transform.translation
                                rot = tf.transform.rotation
                                self._last_tf_query_source = (
                                    f"{exact_source}:bounded_latest"
                                )
                                return (
                                    _quat_xyzw_to_R([rot.x, rot.y, rot.z, rot.w]),
                                    np.array([tr.x, tr.y, tr.z], dtype=np.float64),
                                )
                        except Exception:
                            self._last_tf_query_source = (
                                f"{exact_source}:bounded_latest_failed"
                            )
                    else:
                        self._last_tf_query_source = f"{exact_source}:lookup_failed"
        ...
        def _on_gap_plane(self, msg) -> None:
            ...
            try:
                ...
                self._publish_status(accepted=False, reason="invalid_json")
                return
            if self.require_pose_valid and not bool(data.get("pose_valid", False)):
                self._publish_status(accepted=False, reason="pose_invalid")
                return
            if self.require_metric_3d and not bool(data.get("metric_3d_available", False)):
                self._publish_status(accepted=False, reason="metric_3d_unavailable")
                return
            if self.reject_assumed_depth and bool(data.get("uses_assumed_depth", False)):
                self._publish_status(accepted=False, reason="assumed_depth_rejected")
                return
            ...
            if axis is None or radial is None or center is None:
                ...
                self._publish_status(
                    accepted=False,
                    reason="missing_axis_radial_or_center",
                    camera_frame=str(data.get("frame_id", "")),
                    input_stamp=_stamp_parts_or_none(data.get("stamp")),
                )
                return
            ...
            if not camera_frame:
                ...
                self._publish_status(
                    accepted=False,
                    reason="missing_camera_frame",
                    input_stamp=_stamp_parts_or_none(data.get("stamp")),
                )
                return

            input_stamp = _stamp_parts_or_none(data.get("stamp"))
            resolved = self._resolve_transform(camera_frame, input_stamp)
            if resolved is None:
                self._publish_status(
                    accepted=False,
            ...
            if self.radial_orientation_mode == "base_to_axis":
                axis_unit = _unit(axis_b)
                if axis_unit is None:
                    ...
                    return
                # A circular cylinder does not make roll about its axis
                # observable.  Define a stable robot-relative roll from the
                # base origin toward the closest point on the detected axis.
                radial_from_base = center_b - float(np.dot(center_b, axis_unit)) * axis_unit
                if _unit(radial_from_base) is None:
                    self.get_logger().warn(
                        "base_to_axis radial is degenerate; skipping",
                        throttle_duration_sec=2.0,
                    )
                    self._publish_status(
                        accepted=False,
                        reason="degenerate_base_to_axis_radial",
                        camera_frame=camera_frame,
                        input_stamp=input_stamp,
                    )
                    return
                radial_b = radial_from_base
            elif self.radial_orientation_mode == "horizontal_from_axis":
                axis_unit = _unit(axis_b)
                if axis_unit is None:
                    self._publish_status(
                        accepted=False,
                    ...
                    return
                base_vertical = np.array([0.0, 0.0, 1.0], dtype=np.float64)
                radial_horizontal = np.cross(axis_unit, base_vertical)
                if _unit(radial_horizontal) is None:
                    # A near-vertical pipe has no gravity-defined horizontal
                    # radial; retain the measured mode in that degenerate case.
                    radial_horizontal = radial_b
                elif float(np.dot(radial_horizontal, center_b)) < 0.0:
                    radial_horizontal = -radial_horizontal
                radial_b = radial_horizontal
            ...
            if self.preserve_input_stamp and input_stamp is not None:
                stamp.sec = input_stamp[0]
                stamp.nanosec = input_stamp[1]
